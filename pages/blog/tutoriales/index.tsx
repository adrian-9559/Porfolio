	...
	return (
		...
		>
			...
			<div className="flex items-center justify-between pt-3 border-t border-black/6 dark:border-white/6">
				...
				<span className="text-xs font-semibold text-blue-600 dark:text-blue-400 group-hover:translate-x-0.5 transition-transform">Empezar →</span>
			</div>
		</Link>
	);
}
...
export default function TutorialesPage() {
	...
	const results = useMemo(() => {
		...
		return searchContent(query, "tutorial");
	}, [query]);
			...
			<div className="space-y-10 py-4">
				...
				<div className="space-y-4">
					<div className="flex items-center gap-2">
						...
						<span className="text-xs font-medium text-[#1d1d1f] dark:text-white">Tutoriales</span>
					</div>
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<h1 className="text-3xl md:text-4xl font-bold" style={{ letterSpacing: "-0.03em" }}>Tutoriales</h1>
						</div>
					</div>
					<p className="text-[#6e6e73] dark:text-[#86868b] max-w-xl leading-relaxed">
						Guías paso a paso para aprender tecnologías y conceptos de desarrollo. Desde los fundamentos hasta niveles avanzados.
					</p>
				</div>
				...
				<p className="text-sm text-[#aeaeb2] dark:text-[#636366]">
					{results.length} {results.length === 1 ? "tutorial" : "tutoriales"}
				...
				{results.length === 0 ? (
					<div className="text-center py-20">
						...
						<p className="text-[#6e6e73] dark:text-[#86868b] font-medium">No hay tutoriales para &quot;{query}&quot;</p>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
						{results.map((item) => <TutorialCard key={item.id} item={item} />)}
					</div>
				)}
			</div>
		</BlogLayout>
