...
import { allContent, getContentByType, getContentBySlug, formatDate, ContentMeta } from "@/lib/blog/registry";
...
export default function ArticlePage({ meta, prevMeta, nextMeta }: Props) {
		...
		<BlogLayout>
			<div className="max-w-3xl mx-auto space-y-10 py-4">
				...
				<nav className="flex items-center gap-2 text-xs text-[#aeaeb2] dark:text-[#636366]">
					...
					<span>/</span>
					<Link href="/blog/articulos" className="hover:text-[#6e6e73] dark:hover:text-[#86868b] transition-colors no-underline">Artículos</Link>
					<span>/</span>
				...
				<div className="pt-8 border-t border-black/8 dark:border-white/8 flex items-center justify-between gap-4 flex-wrap">
					...
					<div className="flex items-center gap-4">
						{prevMeta && (
							<Link href={`/blog/articulos/${prevMeta.slug}`} className="text-sm text-[#6e6e73] dark:text-[#86868b] hover:text-[#1d1d1f] dark:hover:text-white transition-colors no-underline truncate max-w-[180px]">
								← {prevMeta.title}
							</Link>
						)}
						{nextMeta && (
							<Link href={`/blog/articulos/${nextMeta.slug}`} className="text-sm text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors no-underline truncate max-w-[180px]">
								{nextMeta.title} →
							</Link>
						)}
					</div>
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
	...
	return {
		props: {
			meta: articles[idx],
			prevMeta: articles[idx - 1] ?? null,
			nextMeta: articles[idx + 1] ?? null,
		},
	};
};
