...
const sections = [
	{
		...
		emoji: "🗄️",
		title: "Base de datos",
		description: "Esquema completo con diagrama interactivo: tablas, columnas, tipos y relaciones FK.",
		badge: "29 tablas",
	...
	return (
		<DefaultLayout>
			<div className="max-w-3xl mx-auto space-y-8">
				<div>
					<h1 className="text-3xl font-bold text-[#1d1d1f] dark:text-white" style={{ letterSpacing: "-0.02em" }}>
						Documentación
					</h1>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					{sections.map(s => (
						<Link key={s.href} href={s.href}
							...
							<p className="text-xs text-[#6e6e73] dark:text-[#86868b] leading-relaxed">{s.description}</p>
							<p className={`text-xs font-semibold mt-3 ${s.text} opacity-0 group-hover:opacity-100 transition-opacity`}>
								Ver documentación →
							</p>
						</Link>
					))}
