  ...
  return (
    <div className="flex flex-wrap gap-2">
      {items.map((item, i) => (
        <span
          key={i}
          className="px-3 py-1 rounded-full text-xs font-medium bg-black/5 dark:bg-white/10 text-[#3d3d3d] dark:text-[#c0c0c5] border border-black/8 dark:border-white/10"
        >
          {item}
        </span>
      ))}
  ...
  return (
    <ul className="space-y-2">
      {items.map((item, i) => (
        ...
        >
          <span
            className={`w-1.5 h-1.5 rounded-full bg-gradient-to-br ${color || "from-violet-500 to-pink-500"} flex-shrink-0 mt-2`}
          />
          {item}
        </li>
      ))}
      ...
      <div className="max-w-4xl mx-auto space-y-12 py-4">
        ...
        <ScrollReveal>
          <nav className="flex items-center gap-2 text-xs text-[#aeaeb2] dark:text-[#636366]">
            <Link
              className="hover:text-[#1d1d1f] dark:hover:text-white transition-colors no-underline"
              href="/CV"
            >
              CV
            </Link>
        ...
        <ScrollReveal>
          <div className="relative overflow-clip rounded-3xl p-8 md:p-10 bg-gradient-to-br from-white to-[#f5f5f7] dark:from-[#111116] dark:to-[#0d0d12] border border-black/8 dark:border-white/8">
            ...
            <div className="relative flex items-start gap-6 flex-wrap">
              <div
                className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${edu.color} flex items-center justify-center text-white shadow-lg flex-shrink-0`}
              >
                {EDU_ICONS[edu.id] || (
                  <span className="text-sm font-bold">{edu.level}</span>
                )}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-4 mt-3 flex-wrap">
                  ...
                  {edu.url && (
                    <a
                      className="flex items-center gap-1.5 text-sm text-[#6e6e73] dark:text-[#86868b] hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
                      href={edu.url}
        ...
        <ScrollReveal>
          <div className="p-6 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8">
            ...
            <BulletList color={edu.color} items={edu.objectives} />
          </div>
        </ScrollReveal>
              ...
              <div className="p-5 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 space-y-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {col.items.map((t, j) => (
                    <span
                      key={j}
                      className="px-2.5 py-1 rounded-full text-xs font-semibold bg-black/5 dark:bg-white/10 text-[#3d3d3d] dark:text-[#c0c0c5] border border-black/8 dark:border-white/10"
                    >
                      {t}
                    </span>
                  ))}
          ...
          <div className="p-6 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {edu.subjects.map((subject, i) => (
                <div
                  key={i}
                  className="flex items-start gap-3 p-3 rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border border-black/5 dark:border-white/5"
                >
                  <div
                    className={`w-1.5 h-1.5 rounded-full bg-gradient-to-br ${edu.color} flex-shrink-0 mt-1.5`}
                  />
                  <div>
                ...
                {edu.projects.map((project, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3 mb-3">
                      ...
                      {project.url && (
                        <a
                          className="flex-shrink-0 w-7 h-7 rounded-lg bg-black/5 dark:bg-white/10 flex items-center justify-center text-[#6e6e73] dark:text-[#86868b] hover:text-violet-600 dark:hover:text-violet-400 transition-colors"
                          href={project.url}
                          rel="noopener noreferrer"
                          target="_blank"
                        >
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <ScrollReveal direction="left">
            <div className="p-6 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 h-full">
              ...
              <BulletList color={edu.color} items={edu.knowledge} />
            </div>
          </ScrollReveal>
          <ScrollReveal direction="right">
            <div className="p-6 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 h-full">
              ...
              <BulletList color={edu.color} items={edu.applications} />
            </div>
          </ScrollReveal>
        </div>
        ...
        {edu.url && (
          <ScrollReveal>
            <a
              className="block p-5 rounded-2xl bg-gradient-to-br from-emerald-500/[0.07] to-teal-400/[0.07] border border-emerald-500/15 hover:border-emerald-500/30 hover:shadow-md hover:shadow-emerald-500/5 transition-all duration-300 group"
              href={edu.url}
              rel="noopener noreferrer"
              target="_blank"
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-bold text-[#1d1d1f] dark:text-white group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                ...
                <svg
                  className="w-5 h-5 text-[#aeaeb2] dark:text-[#636366] group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors flex-shrink-0"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {educationData
                ...
                .map((other) => (
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${other.color} flex items-center justify-center text-white flex-shrink-0 shadow-sm`}
                    >
                      {EDU_ICONS[other.id] || (
                        <span className="text-xs font-bold">{other.level}</span>
                      )}
                    </div>
