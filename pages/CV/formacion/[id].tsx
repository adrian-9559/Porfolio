...
import { educationData, getEducationById, EducationData } from "@/lib/education/data";
		...
		<DefaultLayout>
			<div className="max-w-4xl mx-auto space-y-12 py-4">
				...
				<nav className="flex items-center gap-2 text-xs text-[#aeaeb2] dark:text-[#636366]">
					<Link href="/CV" className="hover:text-[#6e6e73] dark:hover:text-[#86868b] transition-colors no-underline">CV</Link>
					<span>/</span>
				...
				<div className="relative overflow-clip rounded-3xl p-8 md:p-10 bg-gradient-to-br from-white to-[#f5f5f7] dark:from-[#111116] dark:to-[#0d0d12] border border-black/8 dark:border-white/8">
					...
					<div className="flex items-start gap-6 flex-wrap">
						<div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${edu.color} flex items-center justify-center text-white text-sm font-bold shadow-lg flex-shrink-0`}>
							{edu.level}
						</div>
						<div className="flex-1 min-w-0">
							...
							<p className="text-base font-semibold text-blue-600 dark:text-blue-400">{edu.institution}</p>
				...
				<div className="grid md:grid-cols-3 gap-4">
					<div className="p-5 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 space-y-3">
						<h3 className="text-sm font-bold text-[#1d1d1f] dark:text-white">Tecnologías</h3>
						<div className="flex flex-wrap gap-1.5">
							{edu.technologies.map((t, i) => <span key={i} className="tag-chip">{t}</span>)}
						</div>
					</div>
					<div className="p-5 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 space-y-3">
						<h3 className="text-sm font-bold text-[#1d1d1f] dark:text-white">Herramientas</h3>
						<div className="flex flex-wrap gap-1.5">
							{edu.tools.map((t, i) => <span key={i} className="tag-chip">{t}</span>)}
						</div>
					</div>
					<div className="p-5 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 space-y-3">
						<h3 className="text-sm font-bold text-[#1d1d1f] dark:text-white">Metodologías</h3>
						<div className="flex flex-wrap gap-1.5">
							{edu.methodologies.map((m, i) => <span key={i} className="tag-chip">{m}</span>)}
						</div>
					</div>
				</div>
					...
					<Section title="Módulos y materias">
						<div className="grid sm:grid-cols-2 gap-3">
							{edu.subjects.map((subject, i) => (
								<div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-black/2 dark:bg-white/3">
									<div className={`w-1.5 h-1.5 rounded-full bg-gradient-to-b ${edu.color} flex-shrink-0 mt-1.5`} />
									<div>
					...
					<div className="space-y-4">
						...
						<div className="grid md:grid-cols-2 gap-4">
							{edu.projects.map((project, i) => (
								<div key={i} className="group p-5 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 hover:border-black/12 dark:hover:border-white/12 hover:shadow-md transition-all duration-200">
									...
									{project.technologies && (
										<div className="flex flex-wrap gap-1">
											{project.technologies.map((t, j) => (
												<span key={j} className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-black/4 dark:bg-white/6 text-[#6e6e73] dark:text-[#86868b] border border-black/6 dark:border-white/6">
													{t}
												</span>
											))}
										</div>
				...
				<div className="pt-4 border-t border-black/8 dark:border-white/8">
					...
					<div className="grid sm:grid-cols-3 gap-3">
						{educationData.filter((e) => e.id !== edu.id).map((other) => (
							<Link
								key={other.id}
							...
							>
								<div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${other.color} flex items-center justify-center text-white text-xs font-bold flex-shrink-0`}>
									{other.level}
								</div>
					...
					<div className="mt-4">
						<Link href="/CV" className="inline-flex items-center gap-2 text-sm text-[#6e6e73] dark:text-[#86868b] hover:text-[#1d1d1f] dark:hover:text-white transition-colors no-underline">
							<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
