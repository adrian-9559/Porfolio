	...
	return (
		<>
			<header className="sticky top-0 z-50 w-full flex justify-center px-4 pt-3 pb-1 pointer-events-none">
				...
				>
					{/* Logo */}
					<Link href="/" className="flex items-center gap-2 no-underline flex-shrink-0 group">
						<div className="w-7 h-7 rounded-xl bg-gradient-to-br from-blue-500 via-indigo-500 to-violet-600 flex items-center justify-center shadow-sm group-hover:shadow-indigo-400/40 group-hover:shadow-md transition-all duration-300">
							<span className="text-white font-bold text-[13px] tracking-tight">A</span>
						</div>
						<span className="hidden sm:inline text-sm font-semibold text-[#1d1d1f] dark:text-white tracking-tight">
							Adrián<span className="text-[#aeaeb2] dark:text-[#636366] font-normal">.</span>
						</span>
					</Link>
					...
					<div className="flex items-center gap-1.5 flex-shrink-0">
						...
						>
							<span className={`block h-[1.5px] bg-[#1d1d1f] dark:bg-white rounded-full transition-all duration-300 origin-center ${menuOpen ? "w-4 rotate-45 translate-y-[6.5px]" : "w-4"}`} />
							<span className={`block h-[1.5px] bg-[#1d1d1f] dark:bg-white rounded-full transition-all duration-300 ${menuOpen ? "w-0 opacity-0" : "w-3"}`} />
							<span className={`block h-[1.5px] bg-[#1d1d1f] dark:bg-white rounded-full transition-all duration-300 origin-center ${menuOpen ? "w-4 -rotate-45 -translate-y-[6.5px]" : "w-4"}`} />
						</button>
					</div>
				</nav>
