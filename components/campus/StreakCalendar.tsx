...
export function StreakCalendar() {
  ...
  if (loading) {
    return (
      <div className="h-32 rounded-xl bg-[var(--bg-surface)] animate-pulse" />
    );
  }
      ...
      <div className="flex gap-1">
        ...
        <div className="flex flex-col gap-0.5 mr-1">
          {dayLabels.map((d, i) => (
            <span
              key={i}
              className="w-3 h-3 flex items-center justify-center text-[7px] text-[var(--text-muted)]"
            >
              {d}
            </span>
          ))}
            ...
            {week.map((day, di) => (
              <div
                ...
                className={`w-3 h-3 rounded-sm transition-colors ${
                  day.active ? "bg-[var(--accent)]" : "bg-[var(--bg-surface)]"
                }`}
                title={day.date}
              />
            ))}
