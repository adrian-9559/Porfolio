...
type SectionId =
  | "intro" | "variables" | "poo" | "constructores"
  | "stl" | "memoria" | "archivos" | "ejercicios";
...
function Callout({ type, children }: { type: "tip" | "warn" | "danger" | "info"; children: React.ReactNode }) {
  const styles = {
    tip:    "bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-800 text-green-800 dark:text-green-300",
  ...
  const icons = { tip: "✅", info: "💡", warn: "⚠️", danger: "🚨" };
  return (
...
function ExerciseCard({
  num, title, level, description, hint,
}: {
    ...
    <div className="border border-black/10 dark:border-white/10 rounded-2xl overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <span className="w-6 h-6 rounded-full bg-cyan-600 text-white text-xs font-bold flex items-center justify-center shrink-0">{num}</span>
    ...
    <div className="flex gap-8 min-h-[600px]">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex justify-between mt-10 pt-6 border-t border-black/8 dark:border-white/8">
          ...
          <span className="text-xs text-[#aeaeb2] dark:text-[#636366] self-center">{idx+1} / {SECTIONS.length}</span>
          <button
...
function SIntro() {
  return (
    <>
      <H2>¿Qué es C++?</H2>
      <P>C++ es un lenguaje de propósito general creado por Bjarne Stroustrup a principios de los años 80 como extensión de C. Añade programación orientada a objetos (POO), templates (genéricos), gestión automática de recursos (RAII) y la STL. Es un superconjunto de C: casi todo código C válido compila en C++.</P>

      <H3>Diferencias fundamentales con C</H3>
      <div className="overflow-x-auto my-4">
        <table className="text-sm w-full border-collapse">
          <thead><tr className="border-b border-black/10 dark:border-white/10">
            ...
            <th className="text-left py-2 text-[#1d1d1f] dark:text-white">C++ moderno (C++11/17/20)</th>
          </tr></thead>
          <tbody className="text-[#3a3a3c] dark:text-[#aeaeb2] text-xs">
            {[
              ...
              <tr key={c} className="border-b border-black/5 dark:border-white/5">
                <td className="py-1.5 pr-4"><code className="bg-black/5 dark:bg-white/5 px-1 rounded">{c}</code></td>
...
g++ -std=c++17 -O2 -DNDEBUG -o programa main.cpp`}</Code>

      <H3>Paradigma OOP</H3>
      <P>C++ soporta múltiples paradigmas: procedural (como C), orientado a objetos y genérico (templates). La OOP se basa en cuatro pilares:</P>
      <div className="grid grid-cols-2 gap-3 my-4">
        {[
          ["Encapsulación","Agrupar datos y comportamiento en clases, ocultando los detalles internos"],
          ["Herencia","Una clase puede extender otra, reutilizando y especializando su comportamiento"],
          ["Polimorfismo","El mismo código opera sobre diferentes tipos gracias a funciones virtuales"],
          ["Abstracción","Definir interfaces sin exponer la implementación (clases abstractas)"],
        ].map(([title, desc]) => (
          <div key={title} className="border border-cyan-200 dark:border-cyan-800 bg-cyan-50 dark:bg-cyan-950/20 rounded-xl p-3">
            <p className="text-sm font-semibold text-cyan-700 dark:text-cyan-400 mb-1">{title}</p>
...
function SVariables() {
  return (
    <>
      <H2>Variables y tipos modernos</H2>
      <H3>auto — deducción de tipo</H3>
      <Code>{`// auto deduce el tipo del inicializador en COMPILACIÓN (no es Python)
auto entero  = 42;           // int
...
for (const auto& elem : vec) { /* solo lectura */ }`}</Code>
...
void procesar(std::string&& s) {
    ...
    std::string local = std::move(s);
      ...
      <H3>const y constexpr</H3>
      <Code>{`const int MAX = 100;              // no puede modificarse en runtime
...
int  obtener() const;                   // método que no modifica el objeto`}</Code>

      <H3>Tipos de la STL vs primitivos</H3>
      <Code>{`#include <string>
#include <vector>
...
function SPOO() {
  return (
    <>
      ...
      <H3>Clase básica — encapsulación</H3>
      <Code>{`#include <iostream>
#include <string>
using namespace std;
...
class Anfibio : public Animal {
    // puede heredar de dos clases; cuidado con el diamond problem
      ...
      <H3>Polimorfismo</H3>
      <Code>{`// Con punteros o referencias a la clase base → despacho virtual
...
int main() {
    ...
    for (const auto& animal : zoo) {
      ...
      <Code>{`// Clase abstracta — no se puede instanciar directamente
class Figura {
public:
...
function SConstructores() {
  return (
    <>
      <H2>Constructores, destructores y Move Semantics</H2>
...
public:
    ...
    size_t size() const { return tamaño; }
      ...
      <H3>Regla de cinco (Rule of Five)</H3>
      <P>Si defines cualquiera de estos cinco métodos, deberías definir los cinco: destructor, constructor de copia, operador= de copia, constructor de movimiento, operador= de movimiento.</P>
      <Callout type="info">
        En código moderno, prefiere la <strong>Regla de Cero</strong>: usa <code>unique_ptr</code> / <code>shared_ptr</code> como miembros y el compilador genera los cinco métodos correctamente de forma automática.
      </Callout>

      <H3>Lista de inicialización</H3>
      <Code>{`class Punto3D {
...
function SSTL() {
  return (
    <>
      ...
      <P>La STL proporciona contenedores, iteradores y algoritmos genéricos altamente optimizados. Es la herramienta más poderosa del ecosistema C++.</P>
...
// range-for
for (const auto& x : v) cout << x << " ";`}</Code>

      <H3>map y unordered_map</H3>
      <Code>{`#include <map>
...
cout << datos["notas"].size();    // 2`}</Code>

      <H3>set y priority_queue</H3>
      <Code>{`#include <set>
#include <queue>
using namespace std;
...
cout << minpq.top();   // 1 — el menor`}</Code>

      <H3>Lambdas</H3>
      <Code>{`// [captura](parámetros) -> tipo_retorno { cuerpo }
...
function SMemoria() {
  return (
    <>
      <H2>Gestión de memoria dinámica</H2>
      <H3>new y delete — el camino antiguo</H3>
      <Code>{`// Objeto individual
...
int* q = new int[5];
delete q;            // ❌ undefined behavior`}</Code>

      <H3>unique_ptr — propiedad exclusiva</H3>
      <Code>{`#include <memory>
using namespace std;
...
}   // se mueve automáticamente al devolver`}</Code>

      <H3>shared_ptr — propiedad compartida</H3>
      <Code>{`auto sp1 = make_shared<Animal>("Buddy", 4);
{
...
void guardar_datos() {
    ...
    // incluso si aquí hay una excepción, el destructor se llama
      ...
      <Callout type="tip">
        <strong>Regla general</strong>: usa <code>unique_ptr</code> por defecto, <code>shared_ptr</code> cuando necesitas compartir propiedad, y <code>weak_ptr</code> para observar sin poseer. Evita <code>new/delete</code> directo en código de aplicación.
      </Callout>
    </>
  );
}
...
int main() {
    ...
    return 0;
      ...
      <H3>fstream — lectura y escritura</H3>
      <Code>{`#include <fstream>
using namespace std;
...
int main() {
    ...
    return 0;
      ...
      <Callout type="warn">
        Verifica siempre si el archivo se abrió correctamente antes de operar sobre él. Un <code>ifstream</code> que no pudo abrirse simplemente no leerá nada, sin lanzar excepción (a menos que actives excepciones con <code>f.exceptions(...)</code>).
      </Callout>
    </>
  );
}
...
function SEjercicios() {
  const ejercicios = [
    { title: "Sistema de gestión de estudiantes", level: "Básico" as const, description: "Crea una clase Estudiante con nombre, DNI, y vector de notas. Implementa métodos para: añadir nota, calcular media, determinar si está aprobado (media ≥ 5) y mostrar expediente completo. Usa un vector<Estudiante> para gestionar varios.", hint: "Usa const en los métodos que no modifican el estado. El método mostrar puede usar cout sobrecargando operator<<." },
    { title: "Gestor de inventario con STL", level: "Básico" as const, description: "Crea un sistema de inventario para una tienda. Cada producto tiene: nombre, código, precio y stock. Usa un map<string,Producto> (código → producto). Implementa: añadir producto, actualizar stock, buscar por nombre, listar por precio (menor a mayor) y calcular valor total.", hint: "Para listar por precio, copia los valores del map a un vector y ordénalos con sort + lambda." },
    { title: "Mini sistema bancario", level: "Intermedio" as const, description: "Implementa un banco con: Cuenta (base), CuentaCorriente y CuentaAhorro (heredan). CuentaAhorro tiene un interés mensual; CuentaCorriente permite descubierto hasta un límite. Usa polimorfismo para calcular el saldo proyectado a 12 meses de cada cuenta.", hint: "Método virtual puro calcularSaldo(int meses) en la clase base. Las subclases implementan la fórmula diferente." },
    { title: "Grafo con lista de adyacencia", level: "Avanzado" as const, description: "Implementa un grafo no dirigido usando unordered_map<int, vector<int>>. Operaciones: addEdge, removeEdge, hasPath (BFS), shortestPath (BFS) y listar componentes conexas.", hint: "Para BFS usa una queue<int> y un unordered_set<int> como visitados. Para componentes, itera sobre todos los nodos y lanza BFS desde los no visitados." },
  ];

  return (
    <>
      <H2>Ejercicios de C++</H2>
      <P>Proyectos completos para practicar todos los conceptos del tutorial. Cada uno combina varias características del lenguaje.</P>
      ...
      <div className="mt-8 p-4 rounded-2xl bg-cyan-50 dark:bg-cyan-950/20 border border-cyan-200 dark:border-cyan-800">
        <p className="text-sm font-semibold text-cyan-700 dark:text-cyan-400 mb-2">Proyecto final sugerido</p>
        <p className="text-sm text-[#3a3a3c] dark:text-[#aeaeb2]">
          Combina todo en un <strong>sistema de biblioteca</strong>: libros, usuarios, préstamos con fechas, multas por retraso, persistencia en archivos CSV, búsqueda por título/autor/ISBN y notificaciones de devolución próxima.
        </p>
      </div>
    </>
  );
