...
export default function PromptCompressorContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const data = await apiFetch<ToolResult>("/tools/compress-prompt", {
        method: "POST",
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#1d1d1f] dark:text-white mb-2">Prompt Compressor</h1>
        <p className="text-[#6e6e73] dark:text-[#86868b] text-sm">
          Elimina frases de relleno, redundancias y palabras innecesarias de tu prompt sin perder su significado.
          Ideal para reducir el coste por token en APIs como GPT o Claude.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          {loading ? "Comprimiendo…" : "Comprimir prompt"}
        </button>
      </form>
