...
function generatePassword(
	length: number,
	opts: { lowercase: boolean; uppercase: boolean; numbers: boolean; symbols: boolean },
): string {
	let charset = "";
	const guaranteed: string[] = [];

	if (opts.lowercase) { charset += CHARS.lowercase; guaranteed.push(CHARS.lowercase[Math.floor(Math.random() * CHARS.lowercase.length)]); }
	if (opts.uppercase) { charset += CHARS.uppercase; guaranteed.push(CHARS.uppercase[Math.floor(Math.random() * CHARS.uppercase.length)]); }
	if (opts.numbers) { charset += CHARS.numbers; guaranteed.push(CHARS.numbers[Math.floor(Math.random() * CHARS.numbers.length)]); }
	if (opts.symbols) { charset += CHARS.symbols; guaranteed.push(CHARS.symbols[Math.floor(Math.random() * CHARS.symbols.length)]); }
	...
	const result = [...rest];
	guaranteed.forEach((ch, i) => { result[i % length] = ch; });
...
function getStrength(pw: string, opts: { lowercase: boolean; uppercase: boolean; numbers: boolean; symbols: boolean }) {
		...
		<article className="max-w-2xl">
			...
			<div className="space-y-3 mb-10">
				...
				<p className="text-lg text-[#6e6e73] dark:text-[#86868b] leading-relaxed">
					Crea contraseñas seguras y aleatorias al instante. Personaliza la longitud y los tipos de caracteres.
				</p>
			</div>
			...
			<div className="p-6 md:p-8 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 space-y-6">
				...
				<div className="relative">
					<div className="flex items-center gap-3 p-4 rounded-xl bg-[#f5f5f7] dark:bg-[#1c1c22] border border-black/6 dark:border-white/8">
						<p className={`flex-1 font-mono text-base tracking-widest break-all ${password ? "text-[#1d1d1f] dark:text-white" : "text-[#aeaeb2]"}`}>
							{password || "—"}
						</p>
						<button
							onClick={copy}
							...
							{[1, 2, 3, 4].map((s) => (
								<div
									...
									className={`h-1.5 flex-1 rounded-full transition-colors duration-300 ${
										s <= strength.score ? strength.color : "bg-black/8 dark:bg-white/10"
									}`}
								/>
							))}
						</div>
				...
				<div className="space-y-3">
					<div className="flex items-center justify-between">
						<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white">Longitud</p>
						<span className="text-sm font-bold text-blue-600 dark:text-blue-400 tabular-nums">{length}</span>
					</div>
					<input
						type="range"
						min={6}
				...
				{/* Character options */}
				<div className="space-y-3">
					<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white">Incluir</p>
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
						<CheckOption label="Minúsculas (a-z)" checked={opts.lowercase} onChange={(v) => handleOpt("lowercase", v)} />
						<CheckOption label="Mayúsculas (A-Z)" checked={opts.uppercase} onChange={(v) => handleOpt("uppercase", v)} />
						<CheckOption label="Números (0-9)" checked={opts.numbers} onChange={(v) => handleOpt("numbers", v)} />
						<CheckOption label="Símbolos (!@#...)" checked={opts.symbols} onChange={(v) => handleOpt("symbols", v)} />
					</div>
				</div>
			...
			<div className="mt-8 p-5 rounded-2xl bg-blue-50 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/50 space-y-3">
				<p className="text-xs font-semibold text-blue-800 dark:text-blue-300 uppercase tracking-wider">Consejos de seguridad</p>
