...
export default function SQLBuilderContentEn() {
  ...
  return (
    <article aria-label="Interactive SQL Builder" className="max-w-4xl">
      {/* ── Header ── */}
      <div className="space-y-3 mb-6">
        ...
        <p className="text-lg text-[#6e6e73] dark:text-[#86868b] leading-relaxed">
          Build <code className="font-semibold">SELECT</code> queries by
          clicking on tables and columns. No manual SQL required.
        </p>
      </div>
      ...
      <div className="mb-4">
        ...
        >
          ...
          <BookIcon aria-hidden="true" className="w-3.5 h-3.5" /> How to use
        </button>
      </div>
      ...
      {showDocs && (
        ...
        >
          <p className="font-semibold text-[#1d1d1f] dark:text-white flex items-center gap-1.5">
            <BookIcon aria-hidden="true" className="w-4 h-4" /> Quick guide
          </p>
          <ol className="list-decimal pl-4 space-y-1.5">
            ...
            <li>
              Use <strong>Filter</strong>, <strong>Sort</strong>, or{" "}
              <strong>Group</strong> on each column to build your query.
            </li>
            <li>
              For <strong>Filter</strong>, type the value after the{" "}
              <code>=</code>.
            </li>
            <li>
              For <strong>Sort</strong>, each click cycles:{" "}
              <strong>
                OFF{" "}
                <svg
                  aria-hidden="true"
          ...
          <p className="font-semibold text-[#1d1d1f] dark:text-white mt-3 flex items-center gap-1.5">
            <LinkIcon aria-hidden="true" className="w-4 h-4" /> Automatic JOINs
          </p>
          <p>
            If you select columns from tables related by foreign keys (
            <code>REFERENCES</code>), the JOIN is generated automatically. The
            table without a FK acts as the main <code>FROM</code>.
          </p>
          <p className="font-semibold text-[#1d1d1f] dark:text-white mt-3 flex items-center gap-1.5">
            <DocumentIcon aria-hidden="true" className="w-4 h-4" /> Expected DDL
            format
          </p>
          <p>
            Standard <code>CREATE TABLE</code> statements with columns, types,
            inline or standalone <code>PRIMARY KEY</code>, and{" "}
            <code>REFERENCES</code> for foreign keys.
          </p>
        </div>
      )}
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <button
                          aria-label={
                            allSelected
                              ? `Deselect all ${table.name} columns`
                            ...
                            <div key={col.name}>
                              ...
                              >
                                ...
                                <div className="w-4 h-4 flex-shrink-0 flex items-center justify-center">
                                  ...
                                  {col.fk && !col.isPk && (
                                    <LinkIcon
                                      aria-label={`Foreign key to ${col.fk.table}.${col.fk.column}`}
                                ...
                                {/* Toggle badges */}
                                <div
                                  aria-label={`Clauses for ${col.name}`}
      ...
      {selectCols.size > 0 && (
        <div className="mb-4 flex justify-center">
          ...
          >
            <TrashIcon aria-hidden="true" className="w-3.5 h-3.5" /> Start over
          </button>
        </div>
      )}
          ...
          <div className="mt-2 space-y-2">
            ...
            {parseError && (
              ...
              >
                ...
                />{" "}
                Could not detect any tables. Check syntax.
              </p>
            )}
        ...
        >
          ...
          <div className="px-4 py-3 max-h-64 overflow-y-auto">
            {historyLoading ? (
              ...
              <ul aria-label="Saved queries" className="space-y-2" role="list">
                {history.map((item) => (
                  <li
                    key={item.id}
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-[#aeaeb2] dark:text-[#636366] mt-0.5">
                        {relativeTime(item.created_at, "en")}
                      </p>
                    </div>
                    <button
