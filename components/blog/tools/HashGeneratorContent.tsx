...
const ALGORITHMS = [
  { id: "MD5", algorithm: "MD5", label: "MD5" },
  { id: "SHA-1", algorithm: "SHA-1", label: "SHA-1" },
  { id: "SHA-256", algorithm: "SHA-256", label: "SHA-256" },
  { id: "SHA-512", algorithm: "SHA-512", label: "SHA-512" },
] as const;

// MD5 via Web Crypto is not supported — use a pure JS implementation
    ...
    <article className="max-w-3xl">
      <div className="space-y-3 mb-8">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50">
            {t("blog.hashGenerator.tool")}
          </span>
          <span className="text-xs text-[#aeaeb2] dark:text-[#636366]">
            {t("blog.hashGenerator.freeToUse")}
          </span>
        </div>
        <h1
        ...
        >
          {t("blog.hashGenerator.title")}
        </h1>
        <p className="text-lg text-[#6e6e73] dark:text-[#86868b] leading-relaxed">
          {t("blog.hashGenerator.desc")}
        </p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            className="w-full h-32 p-3 text-sm font-mono rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border border-black/8 dark:border-white/8 text-[#1d1d1f] dark:text-white resize-none focus:outline-none focus:border-amber-400 dark:focus:border-amber-600 transition-colors placeholder:text-[#aeaeb2] dark:placeholder:text-[#636366]"
            placeholder={t("blog.hashGenerator.placeholder")}
            value={input}
        ...
        {input && (
          <div className="space-y-2">
            {ALGORITHMS.map((algo) => (
              <div
                key={algo.id}
                className="flex items-center gap-2 p-3 rounded-xl bg-[#f5f5f7] dark:bg-[#1c1c22] border border-black/6 dark:border-white/8"
              >
                <span className="w-20 flex-shrink-0 text-xs font-bold text-[#6e6e73] dark:text-[#86868b] uppercase tracking-wider">
                  {algo.label}
                </span>
                <p className="flex-1 text-xs font-mono text-[#1d1d1f] dark:text-white break-all overflow-hidden">
                  {hashes[algo.id] || "—"}
                </p>
                <button
