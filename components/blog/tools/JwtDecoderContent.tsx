      ...
      <div className="space-y-4">
        ...
        {decoded && (
          <div className="space-y-3">
            ...
            {hasMeta && (
              <div className="p-3 rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border border-black/8 dark:border-white/8 space-y-1.5 text-xs">
                {iss && (
                  <div className="flex gap-2">
                    <span className="font-semibold text-[#6e6e73] dark:text-[#86868b]">
                      {t("blog.jwtDecoder.issuer")}:
                    </span>
                    <span className="text-[#1d1d1f] dark:text-white font-mono">
                      {iss}
                    </span>
                  </div>
                )}
                ...
                {exp !== null && (
                  <div className="flex gap-2">
                    <span className="font-semibold text-[#6e6e73] dark:text-[#86868b]">
                      {t("blog.jwtDecoder.expires")}:
                    </span>
                    <span
                      className={`font-mono ${decoded.expired ? "text-red-600 dark:text-red-400" : "text-[#1d1d1f] dark:text-white"}`}
                    >
                      {formatDate(exp)}
                    </span>
                  </div>
            ...
            <div
              className={`p-4 rounded-xl ${C.header.bg} border ${C.header.border}`}
            >
              <div className="flex items-center justify-between mb-2">
                <p
                  className={`text-xs font-semibold uppercase tracking-wider ${C.header.label}`}
                >
                  {t("blog.jwtDecoder.header")}
                </p>
                <button
                  className="text-xs text-cyan-600 dark:text-cyan-400 hover:underline"
                  onClick={() => copy(jsonStr(decoded.header), "header")}
                >
                  {copied === "header"
                    ? t("blog.jwtDecoder.copied")
                    : t("blog.jwtDecoder.copy")}
                </button>
              </div>
              <pre
                className={`text-xs font-mono whitespace-pre-wrap break-all ${C.header.text}`}
              >
                {jsonStr(decoded.header)}
              </pre>
            </div>

            <div
              className={`p-4 rounded-xl ${C.payload.bg} border ${C.payload.border}`}
            >
              <div className="flex items-center justify-between mb-2">
                <p
                  className={`text-xs font-semibold uppercase tracking-wider ${C.payload.label}`}
                >
                  {t("blog.jwtDecoder.payload")}
                </p>
                <button
                  className="text-xs text-cyan-600 dark:text-cyan-400 hover:underline"
                  onClick={() => copy(jsonStr(decoded.payload), "payload")}
                >
                  {copied === "payload"
                    ? t("blog.jwtDecoder.copied")
                    : t("blog.jwtDecoder.copy")}
              ...
              <pre
                className={`text-xs font-mono whitespace-pre-wrap break-all ${C.payload.text}`}
              >
                {jsonStr(decoded.payload)}
              </pre>
            </div>

            <div
              className={`p-4 rounded-xl ${C.signature.bg} border ${C.signature.border}`}
            >
              <p
                className={`text-xs font-semibold uppercase tracking-wider mb-2 ${C.signature.label}`}
              >
                {t("blog.jwtDecoder.signature")}
              </p>
              <p className={`text-xs font-mono break-all ${C.signature.text}`}>
                {decoded.signature || "\u2014"}
              </p>
            </div>
          </div>
