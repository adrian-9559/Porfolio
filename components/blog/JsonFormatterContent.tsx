			...
			<div className="space-y-4">
				...
				<div className="flex items-center gap-3 flex-wrap">
					<div className="flex items-center gap-2">
						<label className="text-xs font-medium text-[#6e6e73] dark:text-[#86868b]">Indentación:</label>
						{[2, 4].map(n => (
					...
					<div className="ml-auto flex items-center gap-2">
						<button onClick={format} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-amber-500 hover:bg-amber-600 text-white transition-colors">
							Formatear
						</button>
						<button onClick={minify} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-black/8 dark:bg-white/8 text-[#1d1d1f] dark:text-white hover:bg-black/12 dark:hover:bg-white/12 transition-colors">
							Minificar
						</button>
						<button onClick={validate} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-black/8 dark:bg-white/8 text-[#1d1d1f] dark:text-white hover:bg-black/12 dark:hover:bg-white/12 transition-colors">
							Validar
						</button>
						<button onClick={clear} className="px-3 py-1.5 rounded-lg text-xs font-semibold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
							Limpiar
						</button>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
					<div className="space-y-1.5">
						<p className="text-xs font-semibold text-[#6e6e73] dark:text-[#86868b] uppercase tracking-wider">Entrada</p>
						<textarea
							value={input}
					...
					<div className="space-y-1.5">
						<div className="flex items-center justify-between">
							<p className="text-xs font-semibold text-[#6e6e73] dark:text-[#86868b] uppercase tracking-wider">Salida</p>
							{output && !output.startsWith("✅") && (
								<button onClick={copy} className="text-xs text-amber-600 dark:text-amber-400 hover:underline">
