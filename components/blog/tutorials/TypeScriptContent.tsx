...
function SectionIntro() {
  return (
    <>
      <BlogH2>Introducción a TypeScript</BlogH2>
      <BlogP>
        TypeScript es un superconjunto de JavaScript desarrollado por Microsoft
        que añade <strong>tipado estático opcional</strong>. Al compilarse,
        genera código JavaScript estándar que puede ejecutarse en cualquier
        entorno.
      </BlogP>
      <BlogP>
        La clave está en el tipado estático: puedes declarar el tipo de cada
        variable, parámetro de función y valor de retorno, y el compilador te
        avisará de errores <em>antes</em> de ejecutar el código.
      </BlogP>
      <BlogCode>{`// JavaScript (sin tipos)
function sumar(a, b) { return a + b; }
sumar("hola", 5); // "hola5" — bug silencioso!

// TypeScript (con tipos)
function sumar(a: number, b: number): number { return a + b; }
sumar("hola", 5); // Error: Argument of type 'string' is not assignable to 'number'`}</BlogCode>
      <BlogH3>¿Por qué usarlo?</BlogH3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-4">
        {[
          {
            icon: "🐛",
            title: "Menos bugs en runtime",
            desc: "Detecta errores antes de ejecutar el código.",
          },
          {
            icon: "🔍",
            title: "Mejor autocompletado",
            desc: "Tu editor entiende el código y sugiere con precisión.",
          },
          {
            icon: "♻️",
            title: "Refactoring seguro",
            desc: "Cambia nombres y estructuras con confianza.",
          },
          {
            icon: "📄",
            title: "Documentación implícita",
            desc: "Los tipos documentan el código por sí solos.",
          },
        ].map((item, i) => (
          <div
            key={i}
      ...
      <BlogH3>Configuración</BlogH3>
      <BlogCode>{`# Instalar TypeScript
  ...
  "compilerOptions": {
    ...
    "rootDir": "./src"
      ...
      <BlogCallout type="tip">
        Activa <code>strict: true</code> desde el principio. Es más fácil que
        añadirlo después cuando ya tienes mucho código.
      </BlogCallout>
    </>
  );
}
...
function SectionTipos() {
  return (
    <>
      <BlogH2>Tipos básicos</BlogH2>
      <BlogP>
        TypeScript tiene tipos para todos los valores primitivos de JavaScript
        más algunos específicos del lenguaje.
      </BlogP>
      <BlogCode>{`// Primitivos
...
let desconocido: unknown = 42;  // seguro: hay que hacer type guard antes de usar`}</BlogCode>
      <BlogH3>Union Types y Literal Types</BlogH3>
      <BlogCode>{`// Union: acepta varios tipos
...
function procesar(valor: string | number) {
  ...
  return valor.toFixed(2); // aquí sabe que es number
      ...
      <BlogCallout type="warn">
        Evita usar <code>any</code> — elimina todas las ventajas del tipado. Si
        no sabes el tipo, usa <code>unknown</code> y haz un type guard.
      </BlogCallout>
    </>
  );
}
...
function SectionFunciones() {
  return (
    <>
      <BlogH2>Funciones tipadas</BlogH2>
      <BlogP>
        Tipar los parámetros y el valor de retorno de las funciones es donde
        TypeScript aporta más valor, ya que captura errores en el sitio de la
        llamada.
      </BlogP>
      <BlogCode>{`// Función normal
...
function ejecutar(cb: Callback) {
  cb(null, "éxito");
      ...
      <BlogCode>{`// Sobrecargas: misma función, distintas firmas
...
const b = procesar(5);      // number`}</BlogCode>
    </>
  );
}
...
function SectionInterfaces() {
  return (
    <>
      <BlogH2>Interfaces y Type Aliases</BlogH2>
      <BlogP>
        Ambas permiten definir la forma de un objeto, pero tienen diferencias
        clave en su comportamiento.
      </BlogP>
      <BlogCode>{`// Interface — extensible con declaration merging
interface Usuario {
  id: number;
  nombre: string;
  email: string;
...
type UsuarioAdmin = Usuario & { permisos: string[] };`}</BlogCode>
      <BlogH3>¿Interface o Type?</BlogH3>
        ...
        {[
          {
            ...
            items: [
              "Objetos y clases",
              "Declaration merging",
              "Más legible para APIs",
            ],
          },
          {
            label: "Type",
            items: ["Uniones y primitivos", "Mapped types", "Más flexible"],
          },
        ].map((col, i) => (
          <div
            key={i}
      ...
      <BlogCallout type="tip">
        Regla práctica: usa <code>interface</code> para objetos públicos de una
        librería o API y <code>type</code> para todo lo demás.
      </BlogCallout>
    </>
  );
}
...
function SectionGenerics() {
  return (
    <>
      <BlogH2>Genéricos</BlogH2>
      <BlogP>
        Los genéricos permiten crear funciones, clases e interfaces que trabajan
        con <strong>cualquier tipo</strong>, manteniendo la seguridad del
        tipado.
      </BlogP>
      <BlogCode>{`// Función genérica básica
...
function SectionUtilidades() {
  return (
    <>
      ...
      <BlogP>
        TypeScript incluye tipos de utilidad predefinidos que transforman tipos
        existentes de formas muy comunes.
      </BlogP>
...
type ParamsLogin = Parameters<typeof login>; // [string, string]`}</BlogCode>
      <BlogCallout type="tip">
        Los Utility Types son la clave para no repetir definiciones. Si te
        encuentras copiando interfaces, probablemente hay un Utility Type que lo
        resuelve.
      </BlogCallout>
    </>
  );
...
function SEjercicios() {
  return (
    <>
      <BlogH2>Ejercicios de TypeScript</BlogH2>
      <BlogP>
        Practica el sistema de tipos de TypeScript con estos ejercicios. El
        compilador es tu mejor aliado para verificar que tus soluciones son
        correctas.
      </BlogP>
...
// validarProducto({ ...laptop, precio: -10 }); // Error!`}
          title="Interfaz de Producto con validación"
        />
...
// { admin: [Ana, Sara], user: [Luis] }`}
          title="Función genérica groupBy"
        />
...
console.log(pilaNum.size);    // 1`}
          title="Clase Stack genérica tipada"
        />
...
// config.permisos.push("admin"); // Error: ReadonlyArray!`}
          title="Deep Readonly con tipos recursivos"
        />
