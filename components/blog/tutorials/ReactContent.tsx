...
function SectionIntro() {
  return (
    <>
      <BlogH2>Introducción a React</BlogH2>
      <BlogP>
        React es una biblioteca de JavaScript desarrollada por Meta para
        construir interfaces de usuario. Su modelo se basa en componentes
        reutilizables y un flujo de datos unidireccional. La versión 18
        introdujo concurrencia, y React 19 trae hooks de servidor y acciones.
      </BlogP>
      <BlogCode>{`# Crear un proyecto con Vite (recomendado)
...
npx create-next-app@latest mi-app --typescript --tailwind`}</BlogCode>
      <BlogH3>JSX</BlogH3>
      <BlogP>
        JSX es azúcar sintáctico sobre <code>React.createElement</code>. Permite
        escribir HTML dentro de JavaScript de forma expresiva.
      </BlogP>
      <BlogCode>{`// JSX se compila a React.createElement(...)
...
function SectionComponentes() {
  return (
    <>
      <BlogH2>Componentes</BlogH2>
      <BlogH3>Props y children</BlogH3>
      <BlogP>
        Las props son la forma de pasar datos de un componente padre a uno hijo.
        Son inmutables dentro del hijo. <code>children</code> es una prop
        especial que contiene el contenido anidado.
      </BlogP>
      <BlogCode>{`// Tipado de props con TypeScript
...
function Button({
  ...
  return (
    ...
    >
      ...
      <BlogH3>Renderizado de listas</BlogH3>
      <BlogCode>{`interface Producto { id: number; nombre: string; precio: number; }
  ...
  return (
    <ul className="divide-y">
      ...
      <BlogCallout type="warn">
        Nunca uses el índice del array como <code>key</code> si la lista puede
        reordenarse o filtrarse. Usa un identificador único estable.
      </BlogCallout>
    </>
  );
}
...
function SectionHooks() {
  return (
    <>
      <BlogH2>Hooks</BlogH2>
      <BlogH3>useState y useEffect</BlogH3>
...
function PerfilUsuario({ userId }: { userId: number }) {
  ...
  return <div>{usuario.nombre}</div>;
      ...
      <BlogH3>useMemo y useCallback</BlogH3>
  ...
  return (
    <div>
      ...
      <BlogCallout type="tip">
        <code>useMemo</code> y <code>useCallback</code> son optimizaciones: solo
        úsalos cuando tengas problemas de rendimiento medibles. El código sin
        ellos es más simple.
      </BlogCallout>
    </>
  );
...
function SectionRouting() {
  return (
    <>
      <BlogH2>Routing con React Router</BlogH2>
      <BlogCode>{`// main.tsx
...
function App() {
  return (
    <BrowserRouter>
      ...
      <BlogCode>{`// Ruta dinámica con parámetros
function Articulo() {
...
function SectionEstado() {
  return (
    <>
      <BlogH2>Estado global con Context API</BlogH2>
      <BlogP>
        Context API permite compartir estado entre componentes sin pasar props
        manualmente en cada nivel del árbol (prop drilling).
      </BlogP>
...
export function useAuth() {
  ...
  return ctx;
      ...
      <BlogCode>{`// Usar el contexto en cualquier componente
function NavBar() {
...
function SectionOptimizacion() {
  return (
    <>
      <BlogH2>Optimización del renderizado</BlogH2>
      <BlogH3>React.memo</BlogH3>
      <BlogP>
        <code>React.memo</code> evita que un componente se re-renderice si sus
        props no han cambiado. Útil para componentes hijos costosos de
        renderizar.
      </BlogP>
...
function Lista({ items }: { items: Item[] }) {
  ...
  return (
    <div>
      ...
      <BlogCallout type="warn">
        <code>React.memo</code> hace una comparación superficial de props. Si
        pasas objetos o funciones nuevas en cada render, memo no ayudará.
        Combínalo con <code>useCallback</code> y <code>useMemo</code>.
      </BlogCallout>
    </>
  );
}
...
function SectionProyecto() {
  return (
    <>
      <BlogH2>Proyecto Final: Dashboard con estado</BlogH2>
      <BlogP>
        Dashboard completo con Context API para gestión de tareas. Incluye
        filtros, estadísticas y persistencia en localStorage.
      </BlogP>
      <BlogCode>{`// hooks/useTareas.ts
...
export function Dashboard() {
  ...
  return (
    <div className="max-w-2xl mx-auto p-6">
      ...
      <BlogCallout type="tip">
        Para proyectos más grandes, considera Zustand o Redux Toolkit en lugar
        de Context API. Son más eficientes y evitan re-renders innecesarios.
      </BlogCallout>
    </>
  );
}
...
function SEjercicios() {
  return (
    <>
      <BlogH2>Ejercicios de React</BlogH2>
      <BlogP>
        Practica los conceptos de React con estos ejercicios. Cada uno construye
        sobre el anterior para consolidar tu aprendizaje.
      </BlogP>
    ...
    <ul>
      {state.datos.map(u => (
        <li key={u.id}>
          ...
          title="Fetch de datos con useEffect"
        />
...
function useCarrito() {
  return useContext(CarritoContext);
          ...
          title="Context para carrito de compras"
        />
      </div>
    </>
  );
