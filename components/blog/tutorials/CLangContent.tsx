...
function SIntro() {
  return (
    <>
      ...
      <BlogP>
        C es un lenguaje de programación de propósito general, de tipado
        estático y compilado, creado por Dennis Ritchie en los laboratorios Bell
        de AT&T entre 1969 y 1973. Es el lenguaje sobre el que se fundó gran
        parte del software moderno: el kernel de Linux, el intérprete de Python,
        Node.js y decenas de sistemas operativos están escritos en C.
      </BlogP>

      <BlogH3>Historia</BlogH3>
      <BlogP>
        C evolucionó del lenguaje B (Ken Thompson, 1969), que a su vez venía de
        BCPL. Se estandarizó por primera vez como ANSI C (C89/C90), y desde
        entonces ha tenido revisiones: C99 (bucles con declaración en for, tipos{" "}
        <code>bool</code>, <code>_Bool</code>), C11 (threads básicos, generics)
        y C17 (correcciones menores). El estándar en uso hoy en día es
        mayoritariamente C11 o C17.
      </BlogP>

      <BlogH3>Usos reales</BlogH3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-4">
        {[
          ...
          [
            "Sistemas embebidos",
            "Firmware de microcontroladores (Arduino, STM32)",
          ],
      ...
      <BlogH3>Flujo de compilación</BlogH3>
      <BlogP>Un programa en C pasa por cuatro fases antes de ejecutarse:</BlogP>
      ...
      <BlogCode>{`# Compilar con GCC (todo en uno)
...
./programa`}</BlogCode>
      ...
      <BlogH3>Hola, mundo</BlogH3>
      <BlogCode>{`#include <stdio.h>    /* Librería estándar de E/S (printf, scanf...) */
...
int main(void) {
    ...
    return 0;   /* 0 = éxito; cualquier otro valor = error */
      ...
      <BlogCallout type="tip">
        Usa siempre <code>-Wall -Wextra</code> al compilar. Las advertencias del
        compilador te salvarán de muchos bugs.
      </BlogCallout>

      <BlogH3>Diferencias clave con otros lenguajes</BlogH3>
        ...
        <table className="text-sm w-full border-collapse">
          ...
          <tbody className="text-[#3a3a3c] dark:text-[#aeaeb2]">
            {[
              ...
              [
                "~100x más rápido (en operaciones de bajo nivel)",
...
function SVariables() {
  return (
    <>
      <BlogH2>Variables y tipos de datos</BlogH2>
      <BlogP>
        En C debes declarar el tipo de cada variable antes de usarla. El
        compilador reserva exactamente la cantidad de memoria necesaria para
        cada tipo.
      </BlogP>

      <BlogH3>Tipos primitivos</BlogH3>
      <BlogCode>{`/* ENTEROS */
...
void *ptr;            /* puntero genérico */`}</BlogCode>

      <BlogH3>Tamaños con sizeof</BlogH3>
...
/* Salida típica (x86-64):
   ...
   pointer:    8 bytes */`}</BlogCode>
      ...
      <BlogH3>Overflow — ¡peligro silencioso!</BlogH3>
      <BlogP>
        Si un valor supera el rango de su tipo, ocurre <strong>overflow</strong>
        . En tipos con signo, el comportamiento es <em>indefinido</em> en C. En
        tipos sin signo, da la vuelta (módulo 2^N).
      </BlogP>
      <BlogCode>{`#include <stdio.h>
int main(void) {
    unsigned char a = 255;
    ...
    return 0;
      ...
      <BlogH3>Constantes y literales</BlogH3>
      <BlogCode>{`const int MAX = 100;           /* constante tipada */
...
enum Dia hoy = MIE;            /* hoy = 3 */`}</BlogCode>

      <BlogH3>Conversiones de tipo</BlogH3>
...
int truncado = (int)3.99;      /* 3 */`}</BlogCode>

      <BlogCallout type="warn">
        Al convertir <code>double</code> a <code>int</code>, C{" "}
        <strong>trunca</strong> (no redondea). Usa <code>round()</code> de{" "}
        <code>&lt;math.h&gt;</code> si necesitas redondeo.
      </BlogCallout>
    </>
  );
}
...
function SOperadores() {
  return (
    <>
      <BlogH2>Operadores</BlogH2>
      <BlogH3>Aritméticos</BlogH3>
...
printf("%d\\n", c--);     /* 7 — imprime ANTES de decrementar */`}</BlogCode>

      <BlogH3>Relacionales y lógicos</BlogH3>
      <BlogCode>{`/* Relacionales — devuelven 1 (verdadero) o 0 (falso) */
printf("%d\\n", 5 == 5);   /* 1 */
printf("%d\\n", 5 != 4);   /* 1 */
printf("%d\\n", 3 >  2);   /* 1 */
printf("%d\\n", 2 >= 2);   /* 1 */
...
printf("%d\\n", !(x > 0));          /* NOT: 0 */`}</BlogCode>
      ...
      <BlogH3>Bit a bit (Bitwise)</BlogH3>
      <BlogP>
        Los operadores bit a bit trabajan directamente sobre los bits de los
        enteros. Son fundamentales en programación de sistemas, redes y
        hardware.
      </BlogP>
      <BlogCode>{`unsigned int a = 0b1010;   /* 10 en decimal */
...
int activo = (flags >> 3) & 1; /* Leer bit 3 */`}</BlogCode>

      <BlogH3>Asignación compuesta</BlogH3>
      <BlogCode>{`int n = 10;
...
n ^= 5;    /* n = 2  */`}</BlogCode>
      ...
      <BlogH3>Ternario y sizeof</BlogH3>
...
printf("%zu\\n", sizeof(int));      /* 4 */
printf("%zu\\n", sizeof(mayor));    /* 4 */`}</BlogCode>

      <BlogCallout type="tip">
        <strong>Prioridad de operadores</strong> (de mayor a menor):{" "}
        <code>() [] -&gt; .</code> → <code>! ~ ++ -- (cast) *</code> →{" "}
        <code>* / %</code> → <code>+ -</code> → <code>&lt;&lt; &gt;&gt;</code> →{" "}
        <code>&lt; &gt; &lt;= &gt;=</code> → <code>== !=</code> →{" "}
        <code>&amp;</code> → <code>^</code> → <code>|</code> →{" "}
        <code>&amp;&amp;</code> → <code>||</code> → <code>?:</code> →{" "}
        <code>= += …</code>. Cuando tengas dudas, usa paréntesis.
      </BlogCallout>
    </>
  );
}
...
/* ¡IMPORTANTE! Sin break, la ejecución "cae" al siguiente case. */`}</BlogCode>

      <BlogH3>while</BlogH3>
      <BlogCode>{`int i = 1, suma = 0;
while (i <= 100) {
    suma += i;
    i++;
}
printf("Suma 1..100 = %d\\n", suma);   /* 5050 */`}</BlogCode>
      ...
      <BlogCode>{`/* Se ejecuta AL MENOS una vez */
int n;
do {
...
printf("Elegiste: %d\\n", n);`}</BlogCode>
      ...
      <BlogH3>for</BlogH3>
      <BlogCode>{`/* for (inicialización; condición; actualización) */
for (int i = 0; i < 10; i++) {
    printf("%d ", i);
}
...
for (int i = 0, j = 10; i < j; i++, j--) {
    printf("i=%d j=%d\\n", i, j);
      ...
      <BlogH3>break, continue y goto</BlogH3>
      <BlogCode>{`/* break — sale del bucle más cercano */
for (int i = 0; i < 10; i++) {
    if (i == 5) break;
...
printf("Salimos\\n");`}</BlogCode>
      ...
      <BlogCallout type="tip">
        Evita <code>goto</code> en código general. Su único uso aceptado es
        salir de bucles profundamente anidados cuando <code>break</code> no
        alcanza.
      </BlogCallout>
    </>
  );
...
function SFunciones() {
  return (
    <>
      ...
      <BlogH3>Declaración, definición y llamada</BlogH3>
      <BlogCode>{`#include <stdio.h>
...
double potencia(double base, int exp) {
    ...
    return resultado;
      ...
      <BlogH3>Paso por valor vs paso por referencia</BlogH3>
      <BlogCode>{`/* Paso por valor — el parámetro es una COPIA */
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Scope (ámbito)</BlogH3>
      <BlogCode>{`int global = 100;   /* visible en todo el archivo */
...
/* 1ª llamada: 1, 2ª: 2, 3ª: 3... */`}</BlogCode>

      <BlogH3>Recursividad</BlogH3>
      <BlogCode>{`/* Factorial iterativo vs recursivo */
...
int main(void) {
    ...
    return 0;
      ...
      <BlogCallout type="warn">
        La recursividad sin caso base produce desbordamiento de pila (
        <em>stack overflow</em>). Siempre define un <strong>caso base</strong>{" "}
        claro.
      </BlogCallout>
    </>
  );
...
function SArrays() {
  return (
    <>
      <BlogH2>Arrays y strings</BlogH2>
      <BlogH3>Arrays unidimensionales</BlogH3>
      <BlogCode>{`#include <stdio.h>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Memoria en arrays</BlogH3>
      <BlogP>
        Los elementos de un array se almacenan de forma{" "}
        <strong>contigua</strong> en memoria. Si <code>int arr[5]</code>{" "}
        comienza en la dirección 0x100, los elementos están en 0x100, 0x104,
        0x108, 0x10C, 0x110 (en un sistema de 4 bytes por int).
      </BlogP>
...
/* arr[1] = arr + sizeof(int) = arr + 4 bytes */`}</BlogCode>
      ...
      <BlogH3>Strings en C</BlogH3>
      <BlogP>
        En C, un string es simplemente un array de <code>char</code> terminado
        en el carácter nulo <code>'\0'</code>. Este terminador es obligatorio:
        sin él, las funciones de string corren fuera de los límites.
      </BlogP>
      <BlogCode>{`#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Errores comunes con strings</BlogH3>
      <BlogCode>{`/* ❌ PELIGRO — buffer overflow */
char buf[10];
...
fgets(linea, sizeof(linea), stdin);   /* incluye el '\\n' al final */`}</BlogCode>
      ...
      <BlogCallout type="danger">
        Nunca uses <code>gets()</code> — fue eliminada en C11 porque no limita
        el tamaño de entrada. Usa siempre <code>fgets()</code>.
      </BlogCallout>
    </>
  );
}
...
function SPunteros() {
  return (
    <>
      <BlogH2>Punteros</BlogH2>
      <BlogP>
        Un puntero es una variable que almacena la{" "}
        <strong>dirección de memoria</strong> de otra variable. Son la
        característica más poderosa de C y la fuente de muchos bugs si se usan
        mal.
      </BlogP>

      <BlogH3>Concepto de memoria</BlogH3>
      <BlogP>
        La memoria RAM se puede imaginar como un gran array de bytes, donde cada
        byte tiene una dirección única. Cuando declaras <code>int x = 42</code>,
        el sistema reserva 4 bytes consecutivos en alguna dirección, por ejemplo
        0x7fff5abc, y almacena 42 en esa posición.
      </BlogP>

      <BlogH3>Declaración y operadores básicos</BlogH3>
      <BlogCode>{`#include <stdio.h>
...
int main(void) {
    ...
    *ptr = 100;          /* modifica valor a través del puntero */
    ...
    return 0;
      ...
      <BlogH3>Aritmética de punteros</BlogH3>
...
for (int *q = arr; q < arr + 5; q++) {
    printf("%d ", *q);   /* 10 20 30 40 50 */
      ...
      <BlogH3>Relación entre arrays y punteros</BlogH3>
...
/* DIFERENCIA IMPORTANTE:
   ...
   - int *p = arr; p es una variable (puedes hacer p++) */`}</BlogCode>

      <BlogH3>Punteros a punteros</BlogH3>
      <BlogCode>{`int   n   = 5;
int  *p   = &n;
...
void asignar(int **dest, int *src) {
    *dest = src;   /* modifica el puntero original */
      ...
      <BlogH3>Punteros y memoria dinámica</BlogH3>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Errores críticos con punteros</BlogH3>
      <BlogCode>{`/* ❌ Puntero sin inicializar (wild pointer) */
int *p;
...
int *mala(void) {
    ...
    return &local;   /* local se destruye al salir de la función */
      ...
      <BlogCallout type="danger">
        Compila siempre con <code>-fsanitize=address,undefined</code> durante el
        desarrollo. AddressSanitizer detecta use-after-free, buffer overflows y
        otros errores de memoria en tiempo de ejecución.
      </BlogCallout>
    </>
  );
}
...
function SStructs() {
  return (
    <>
      ...
      <BlogP>
        Un <code>struct</code> es un tipo de dato compuesto que agrupa variables
        relacionadas bajo un mismo nombre. Son la base para crear abstracciones
        de datos en C.
      </BlogP>

      <BlogH3>Creación y uso básico</BlogH3>
      <BlogCode>{`#include <stdio.h>
#include <string.h>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>typedef — alias de tipo</BlogH3>
      <BlogCode>{`/* Sin typedef — hay que escribir "struct" siempre */
struct Punto p;
...
typedef int (*Comparador)(const void *, const void *);`}</BlogCode>

      <BlogH3>Structs y punteros</BlogH3>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Arrays de structs — lista de datos</BlogH3>
      <BlogCode>{`#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(void) {
    ...
    return 0;
      ...
      <BlogH3>Structs anidados y struct recursivo</BlogH3>
      <BlogCode>{`typedef struct Nodo {
    int          valor;
  ...
  const ejerciciosBasicos = [
    {
      ...
      description:
        "Implementa una función para cada operación aritmética (+, -, *, /) que reciba dos doubles y devuelva el resultado. Muestra un menú de selección de operación.",
      hint: "Usa un switch para seleccionar la operación. Verifica que el divisor no sea cero.",
      solution: `double dividir(double a, double b) {
    if (b == 0.0) {
    ...
    {
      title: "Inversión de array",
      description:
        "Recibe un array de enteros y su tamaño. Inviértelo in-place sin usar un array auxiliar.",
      hint: "Usa dos índices: uno al inicio y otro al final. Intercámbialos y avanza hacia el centro.",
      solution: `void invertir(int arr[], int n) {
    ...
    },
    {
      title: "FizzBuzz",
      description:
        "Imprime los números del 1 al 100. Para múltiplos de 3 imprime 'Fizz', para múltiplos de 5 imprime 'Buzz', para múltiplos de ambos imprime 'FizzBuzz'.",
      hint: "Comprueba primero el caso FizzBuzz (múltiplo de 15), luego Fizz, luego Buzz.",
    ...
    },
    {
      ...
      description:
        "Dada un número entero positivo, calcula la suma de sus dígitos. Ejemplo: 1234 → 1+2+3+4 = 10.",
      hint: "Usa el operador módulo (%) para extraer el último dígito y la división entera (/) para eliminar el último dígito.",
      solution: `int suma_digitos(int n) {
    int suma = 0;
    while (n > 0) {
        suma += n % 10;
        n /= 10;
    ...
    },
    {
      title: "Número primo",
      description:
        "Implementa una función que determine si un número es primo. Devuelve 1 si es primo, 0 si no.",
      hint: "Solo necesitas verificar divisores hasta la raíz cuadrada del número.",
...
int es_primo(int n) {
    ...
    },
    {
      ...
      description:
        "Dada una cadena de caracteres, cuenta el número de vocales (a, e, i, o, u, mayúsculas y minúsculas).",
      hint: "Usa strchr() o compara cada carácter contra las vocales. Convierte a minúsculas con tolower() de <ctype.h>.",
...
int contar_vocales(const char *s) {
    ...
    },
    {
      title: "Palíndromo",
      description:
        "Determina si una cadena es un palíndromo (se lee igual al derecho que al revés). Ignora mayúsculas y espacios.",
      hint: "Compara el primer y último carácter, avanzando hacia el centro.",
...
int es_palindromo(const char *s) {
    ...
    },
    {
      ...
      description:
        "Imprime la tabla de multiplicar del 1 al 10 en formato de cuadrícula alineado.",
      hint: 'Usa printf con ancho de campo fijo: printf("%4d", valor).',
    ...
    },
    {
      ...
      description:
        "Implementa el algoritmo de Euclides para calcular el MCD de dos enteros.",
      hint: "El algoritmo de Euclides: mcd(a, b) = mcd(b, a % b) mientras b != 0.",
      solution: `int mcd(int a, int b) {
    while (b != 0) {
        int temp = b;
        b = a % b;
        a = temp;
    ...
    },
    {
      ...
      description:
        'Convierte un número binario representado como string ("1101") a su valor decimal.',
      hint: "Recorre la cadena de izquierda a derecha. Cada posición equivale a 2^(n-1-i).",
      solution: `int bin_a_dec(const char *bin) {
    int result = 0;
    for (; *bin; bin++) {
  ...
  const ejerciciosIntermedios = [
    {
      ...
      description:
        "Implementa el algoritmo Insertion Sort para ordenar un array de enteros de menor a mayor.",
      hint: "Para cada elemento, desplaza los mayores una posición a la derecha e inserta en su lugar correcto.",
    ...
    },
    {
      ...
      description:
        "Implementa búsqueda binaria sobre un array ordenado. Devuelve el índice del elemento o -1 si no existe.",
      hint: "Mantén un rango [izq, der]. Compara el elemento del medio con el buscado y reduce el rango a la mitad.",
      solution: `int busqueda_binaria(int arr[], int n, int objetivo) {
    int izq = 0, der = n - 1;
    while (izq <= der) {
    ...
    },
    {
      ...
      description:
        "Dada una matriz cuadrada N×N, calcula su transpuesta in-place (intercambia filas por columnas).",
      hint: "Solo itera sobre el triángulo superior (j > i) e intercambia arr[i][j] con arr[j][i].",
    },
    {
      title: "Lista enlazada — operaciones",
      description:
        "Implementa una lista enlazada simple con: insertar al final, buscar por valor, eliminar por valor y mostrar todos.",
      hint: "Necesitarás tres punteros típicamente: anterior, actual, nuevo. Cuida el caso de eliminar la cabeza.",
    },
    {
      title: "Pila con array",
      description:
        "Implementa una pila (LIFO) usando un array estático. Operaciones: push, pop, peek, isEmpty.",
      hint: "Usa una variable 'top' que empiece en -1. Push incrementa top, pop lo decrementa.",
...
int pop(Pila *p, int *v) {
    ...
    *v = p->data[p->top--]; return 1;
    ...
    {
      title: "Cola con array circular",
      description:
        "Implementa una cola (FIFO) con array circular. Operaciones: enqueue, dequeue, isEmpty, isFull.",
      hint: "Usa índices 'frente' y 'trasero' con módulo para crear el efecto circular.",
    },
    {
      title: "Contar palabras",
      description:
        "Dada una cadena de texto, cuenta el número de palabras separadas por espacios, tabulaciones o saltos de línea.",
      hint: "Un estado simple de dos estados: dentro/fuera de palabra. Incrementa el contador al entrar en una palabra.",
      solution: `#include <ctype.h>
int contar_palabras(const char *s) {
    ...
    },
    {
      ...
      description:
        "Multiplica dos matrices A (M×K) y B (K×N) y almacena el resultado en C (M×N).",
      hint: "Para C[i][j], suma los productos A[i][k]*B[k][j] para k de 0 a K-1.",
    },
    {
      title: "Comprimir RLE",
      description:
        "Implementa compresión Run-Length Encoding (RLE): 'AAABBBCC' → '3A3B2C'.",
      hint: "Recorre la cadena contando caracteres consecutivos iguales.",
    },
    {
      title: "Histograma de caracteres",
      description:
        "Dado un texto, muestra cuántas veces aparece cada letra del alfabeto (sin distinguir mayúsculas).",
      hint: "Usa un array de 26 enteros indexado por (tolower(c) - 'a').",
    },
  ];

  const ejerciciosAvanzados = [
    {
      title: "Tabla hash simple",
      description:
        "Implementa una tabla hash con encadenamiento para almacenar pares clave-valor (strings → enteros). Operaciones: insertar, buscar, eliminar.",
      hint: "Función hash: suma de códigos ASCII × primo, módulo tamaño de tabla. Cada bucket es una lista enlazada de pares.",
    },
    {
      title: "Árbol binario de búsqueda",
      description:
        "Implementa un ABB con insertar, buscar, recorrido inorden (da el árbol ordenado) y eliminar nodo.",
      hint: "Para eliminar: caso 1 (sin hijos), caso 2 (un hijo), caso 3 (dos hijos → reemplaza con el menor del subárbol derecho).",
    },
    {
      title: "Analizador de expresiones matemáticas",
      description:
        "Dada una cadena con una expresión matemática simple (operadores +, -, *, /, paréntesis), evalúala respetando la precedencia de operadores.",
      hint: "Implementa un parser recursivo descendente. Gram.: expr → term ((+|-) term)*, term → factor ((*|/) factor)*, factor → número | (expr).",
    },
    {
      title: "Gestor de memoria simple",
      description:
        "Implementa un allocator básico: una región de memoria fija, con malloc y free propios que gestionan bloques con cabeceras de metadatos.",
      hint: "Cada bloque tiene una cabecera con tamaño y flag 'libre'. malloc busca el primer bloque libre suficientemente grande, free lo marca como libre y fusiona bloques adyacentes.",
    },
    {
      title: "Compresión Huffman",
      description:
        "Implementa el algoritmo de Huffman para comprimir un texto: construye el árbol de frecuencias, genera los códigos y codifica el texto.",
      hint: "Usa una cola de prioridad (min-heap) de nodos. Cada paso: extrae los dos de menor frecuencia, crea un nodo padre y reinserta.",
    },
  ];
  ...
  return (
    <>
      ...
      <BlogP>
        Practica con estos ejercicios ordenados por dificultad. Intenta
        resolverlos antes de mirar la pista o solución.
      </BlogP>

      <BlogH3>Básicos (1–10)</BlogH3>
      ...
      <BlogH3>Intermedios (11–20)</BlogH3>
      <div className="space-y-2 mb-8">
      ...
      <BlogCallout type="tip">
        Para los ejercicios avanzados, busca primero la teoría necesaria
        (árboles, algoritmos de compresión) antes de implementar. La práctica de
        leer código de otros y entender estructuras es tan importante como
