  ...
  return (
    <article className="max-w-3xl">
      ...
      <p className="text-base md:text-lg text-[#6e6e73] dark:text-[#86868b] leading-relaxed mb-8">
        Learn how to run language models locally with Ollama and connect them to
        MCP tools to supercharge your AI agents without relying on the cloud.
      </p>
      ...
      <BlogP>
        ...
        use real tools (files, terminal, APIs) through the same standard that
        cloud models use.
      </BlogP>
      <BlogUl>
        <BlogLi>
          <strong>Total privacy</strong> — your data never leaves your computer.
          Ideal for sensitive code, personal documents, or projects under NDA
        </BlogLi>
        <BlogLi>
          <strong>Zero cost</strong> — no API tokens, no subscriptions. Use your
          GPU or CPU as much as you want
        </BlogLi>
        <BlogLi>
        ...
        <BlogLi>
          <strong>Same protocol</strong> — the MCPs you configure for OpenAI or
          Anthropic work the same with Ollama. You change the backend, not the
          tools
        </BlogLi>
      </BlogUl>
      ...
      <BlogP>
        Ollama is the simplest local model manager. On macOS you install it with
        Homebrew:
      </BlogP>
      <BlogCode>{`brew install ollama
ollama serve`}</BlogCode>
      <BlogP>
        Once running, download the models you need. For tool calling you need
        models with good function support:
      </BlogP>
...
ollama pull deepseek-coder`}</BlogCode>
      <BlogP>
        You can verify it works with <code>ollama list</code>. Ollama exposes an
        API at <code>http://localhost:11434</code> compatible with OpenAI.
      </BlogP>
  ...
  "mcpServers": {
    ...
    "github": {
      ...
      <BlogP>
        With this, OpenCode uses the local model for all conversations and the
        MCPs give it access to the file system and GitHub. Every time the AI
        needs to read a file or list a directory, it will automatically call the
        filesystem server.
      </BlogP>

      <BlogH2>Continue.dev + Ollama</BlogH2>
      <BlogP>
        Continue.dev is the open source assistant for VS Code and JetBrains. It
        supports Ollama as a backend and MCPs as a tool source. The
        configuration goes in <code>.continuerc.json</code> or in the settings
        UI:
      </BlogP>
      <BlogCode>{`{
  "models": [
  ...
  "tabAutocompleteModel": {
    ...
    "model": "starcoder2:3b"
      ...
      <BlogP>
        ...
        chat, and the ability to run MCP tools directly from the editor.
      </BlogP>
  ...
  "mcpServers": {
    ...
    "github": {
      ...
      <BlogP>
        Imagine you just cloned a project and want to understand its structure
        without reading file by file. With Ollama + Filesystem MCP you can ask
        the AI:
      </BlogP>
...
what stack it uses, and how to run it."`}</BlogCode>
      ...
      <BlogP>
        Not all local models handle tool calling well. These are the ones that
        work best with MCP:
      </BlogP>
      <BlogUl>
        <BlogLi>
        ...
        <BlogLi>
          ...
          technical responses are detailed, though it sometimes loses precision
          in multi-tool calls
        </BlogLi>
        <BlogLi>
          <strong>Mistral (7B)</strong> — fast and consistent. Good tool calling
          support, ideal as a versatile option when switching between code and
          general tasks
        </BlogLi>
      </BlogUl>
      ...
      <BlogUl>
        <BlogLi>
          <strong>Tool calling consistency</strong> — local models don't always
          decide to call the right tool. Sometimes they ignore an available MCP
          or call the wrong tool. Cloud models (GPT-4o, Claude) are more
          reliable in this aspect
        </BlogLi>
        <BlogLi>
          <strong>Context window</strong> — local models have smaller contexts
          (4K-32K tokens). If the MCP returns a lot of information, the model
          may forget previous instructions
        </BlogLi>
        <BlogLi>
          <strong>Speed</strong> — without a dedicated GPU, 7B models can take
          seconds to respond. In the cloud you get responses in milliseconds.
          For long tasks with multiple MCP calls the difference is noticeable
        </BlogLi>
        <BlogLi>
          <strong>Small models</strong> — 3B or 7B models understand simple
          tools but get confused with complex workflows requiring multiple
          chained tools
        </BlogLi>
      </BlogUl>
      ...
      <BlogCallout type="tip">
        ...
        <strong>Qwen2.5-Coder:7B</strong> as your main model. If your machine is
        modest, use <strong>Llama 3.2:3B</strong> for quick tasks and switch to
        Qwen when you need precision. For complex tool calling, combine it with
        a cloud model as a fallback.
      </BlogCallout>
    </article>
  );
}
