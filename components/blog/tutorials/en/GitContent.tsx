...
const SECTIONS: SectionDef[] = [
  { id: "intro", label: "1. What is Git?" },
  { id: "config", label: "2. Initial setup" },
  { id: "basico", label: "3. Basic commands" },
  { id: "ramas", label: "4. Branches" },
  { id: "remoto", label: "5. Remote repositories" },
  { id: "conflictos", label: "6. Conflict resolution" },
  { id: "avanzado", label: "7. Advanced commands" },
  { id: "flujos", label: "8. Workflows" },
  { id: "referencia", label: "9. Quick reference" },
];
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-4">
        {[
          ...
          {
            term: "Commit",
            def: "Snapshot (snapshot) del estado del proyecto en un momento concreto.",
          },
          {
            term: "Branch",
        ...
        {[
          {
            icon: "⚡",
            title: "Velocidad",
            text: "Almost all operations are local, without network latency.",
          },
          {
            icon: "🔒",
            title: "Integridad",
            text: "Each commit has a unique SHA-1 hash. Nada puede modificarse sin que Git lo detecte.",
          },
          {
            icon: "🌿",
            title: "Ramificación",
            text: "Creating and switching branches is instant. Fomenta trabajar en ramas por feature.",
          },
        ].map(({ icon, title, text }) => (
          <div
            key={title}
...
function SectionConfig() {
  return (
    <>
      <BlogH2>Initial setup</BlogH2>
      <BlogP>
        Before making any commit, Git needs to know who you are. Esta
        información se incluye en cada commit que hagas.
      </BlogP>

      <BlogH3>Global identity</BlogH3>
      <BlogCode>{`git config --global user.name "Tu Nombre"
git config --global user.email "tu@email.com"`}</BlogCode>

      <BlogH3>Default editor</BlogH3>
      <BlogCode>{`# VS Code
...
git config --global alias.staged "diff --cached"`}</BlogCode>
      ...
      <BlogH3>Per-project configuration</BlogH3>
      <BlogP>
...
git config user.email "yo@empresa.com"`}</BlogCode>
      ...
      <BlogH3>View configuration</BlogH3>
...
*.swp`}</BlogCode>
      <BlogCallout type="danger">
        Don't upload files <strong>.env</strong> con credenciales reales.
...
function SectionBasico() {
  return (
    <>
      <BlogH2>Basic commands</BlogH2>

      <BlogH3>Initialize and clone</BlogH3>
...
git clone -b feature/login https://github.com/usuario/repo.git`}</BlogCode>
      ...
      <BlogH3>Status and differences</BlogH3>
...
git diff rama1 rama2     # Diferencias entre dos ramas`}</BlogCode>
      ...
      <BlogH3>Staging and commits</BlogH3>
...
git revert HEAD                    # Revertir el último commit`}</BlogCode>
      ...
      <BlogH3>Stash — save changes temporarily</BlogH3>
...
git stash branch feature/nueva     # Crear rama desde el stash`}</BlogCode>
      ...
      <BlogH3>Tagging versions (tags)</BlogH3>
...
function SectionRamas() {
  return (
    <>
      ...
      <BlogH3>Branch management</BlogH3>
...
git branch -m viejo nuevo          # Renombrar cualquier rama`}</BlogCode>
      ...
      <BlogH3>Merge — merging branches</BlogH3>
      <BlogP>
...
#                    └──C──D──┘`}</BlogCode>
      ...
      <BlogH3>Rebase — rewriting history</BlogH3>
      <BlogP>
...
git rebase --abort`}</BlogCode>
      ...
      <BlogH3>Cherry-pick — copy specific commits</BlogH3>
...
function SectionRemoto() {
  return (
    <>
      <BlogH2>Remote repositories</BlogH2>

      <BlogH3>Manage remotes</BlogH3>
      <BlogCode>{`git remote                           # View configured remotes
git remote -v                        # View fetch and push URLs
...
git remote set-url origin https://...# Cambiar URL`}</BlogCode>
      ...
      <BlogH3>Fetch, pull and push</BlogH3>
...
git switch -c feature/x origin/feature/x  # versión moderna`}</BlogCode>
      ...
      <BlogH3>Working with forks</BlogH3>
...
function SectionConflicts() {
  const [demo, setDemo] = useState<"none" | "markers" | "resolved">("none");

  return (
    <>
      <BlogH2>Conflict resolution</BlogH2>
      <BlogP>
      ...
      <BlogH3>Conflict markers</BlogH3>
      <BlogP>
...
git cherry-pick --abort`}</BlogCode>
      ...
      <BlogH3>Conflict resolution strategies</BlogH3>
...
git mergetool src/utils.js`}</BlogCode>
      ...
      <BlogH3>Conflicts en rebase</BlogH3>
      <BlogP>
...
# Repetir hasta que todos los commits estén aplicados`}</BlogCode>
      ...
      <BlogH3>Conflict prevention</BlogH3>
      ...
      <BlogH3>Merge strategy configuration</BlogH3>
...
git config --global pull.rebase true  # Siempre rebase`}</BlogCode>
      ...
      <BlogH3>Per-file merge attributes</BlogH3>
...
function SectionAvanzado() {
  return (
    <>
      <BlogH2>Advanced commands</BlogH2>

      <BlogH3>git bisect — find the commit that introduced a bug</BlogH3>
      <BlogP>
...
git bisect run npm test          # Bueno si sale 0, malo si sale != 0`}</BlogCode>
      ...
      <BlogH3>git reflog — the safety net</BlogH3>
      <BlogP>
...
git reset --hard HEAD@{1}         # Volver a ese estado (si estás seguro)`}</BlogCode>
      ...
      <BlogH3>git worktree — multiple branches at once</BlogH3>
      <BlogP>
...
git worktree remove ../hotfix`}</BlogCode>
      ...
      <BlogH3>git blame — who wrote what</BlogH3>
...
git blame --follow archivo.js      # Seguir el archivo si fue renombrado`}</BlogCode>
      ...
      <BlogH3>git clean — clean untracked files</BlogH3>
...
git clean -fdx                     # Archivos ignorados y no trackeados`}</BlogCode>
      ...
      <BlogH3>git submodule — dependencies as repos</BlogH3>
...
git submodule status`}</BlogCode>
      ...
      <BlogH3>Granular staging manipulation</BlogH3>
...
git restore --staged -p archivo.js`}</BlogCode>
      ...
      <BlogH3>Mass history rewriting</BlogH3>
...
function SectionFlujos() {
  return (
    <>
      <BlogH2>Workflows (Workflows)</BlogH2>
      <BlogP>
...
if (featureFlags.isEnabled('nuevo-pago')) {
  ...
  // código antiguo (siempre llega a producción)
      ...
      <BlogH3>Branch naming convention</BlogH3>
      <CmdTable
        rows={[
          ["feature/nombre-corto", "New feature"],
          ["fix/descripcion-bug", "Bug fix"],
          ["hotfix/critico", "Urgent production fix"],
          ["release/1.2.0", "Release preparation"],
          ["chore/actualizar-deps", "Mantenimiento (sin cambio funcional)"],
          ["docs/readme-setup", "Documentation"],
          ["refactor/auth-service", "Refactoring without functional change"],
          ["test/login-unit-tests", "Tests"],
        ]}
      />

      <BlogH3>Commit message convention (Conventional Commits)</BlogH3>
      <BlogP>
...
function SectionReferencia() {
  return (
    <>
      <BlogH2>Quick reference</BlogH2>

      <BlogH3>Commands esenciales del día a día</BlogH3>
      <CmdTable
        rows={[
      ...
      <BlogH3>Branches and remotes</BlogH3>
      <CmdTable
        rows={[
      ...
      <BlogH3>Conflict resolution</BlogH3>
      <CmdTable
        rows={[
      ...
      <BlogH3>Inspection and search</BlogH3>
      <CmdTable
        rows={[
          [
      ...
      <BlogH3>File status states</BlogH3>
...
UU  = Unmerged (conflicto sin resolver)`}</BlogCode>
      ...
      <BlogCallout type="tip">
        Add the alias <BlogInlineCode>git lg</BlogInlineCode> configurado antes
