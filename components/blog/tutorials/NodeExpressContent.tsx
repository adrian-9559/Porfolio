...
function SectionIntro() {
  return (
    <>
      <BlogH2>Introducción a Node.js y Express</BlogH2>
      <BlogP>
        Node.js es un entorno de ejecución de JavaScript sobre el motor V8 de
        Chrome. Usa un modelo de E/S no bloqueante y orientado a eventos, ideal
        para APIs y aplicaciones en tiempo real.
      </BlogP>
      <BlogP>
        Express es el framework web más popular para Node.js. Minimalista y
        flexible, ofrece routing, middlewares y una gran cantidad de plugins del
        ecosistema.
      </BlogP>
      <BlogCode>{`# Inicializar proyecto
npm init -y
npm install express
...
# tsconfig.json básico
{
  "compilerOptions": {
    ...
    "esModuleInterop": true
      ...
      <BlogCode>{`// src/index.ts - Servidor mínimo
...
function SectionExpressBasico() {
  return (
    <>
      <BlogH2>Express: routing y middlewares</BlogH2>
      <BlogH3>Routing</BlogH3>
      <BlogP>
        Express organiza las rutas por método HTTP y patrón de URL. Puedes usar{" "}
        <code>Router</code> para agrupar rutas relacionadas.
      </BlogP>
...
export default router;`}</BlogCode>
      ...
      <BlogP>
        Un middleware es una función que recibe <code>(req, res, next)</code>.
        Puede modificar la solicitud/respuesta o pasar el control al siguiente
        middleware con <code>next()</code>.
      </BlogP>
...
app.use(logger);
app.use("/api", router);
app.use(manejarErrores); // debe ir al final`}</BlogCode>
    </>
  );
}
...
function SectionRestApi() {
  return (
    <>
      <BlogH2>Construyendo una REST API</BlogH2>
      <BlogH3>CRUD completo</BlogH3>
      <BlogCode>{`// routes/tareas.router.ts
...
export default router;`}</BlogCode>
      <BlogCode>{`// controllers/tareas.controller.ts
  ...
  eliminar = async (req: Request, res: Response) => {
    ...
    res.status(204).send();
      ...
      <BlogCallout type="tip">
        Usa los códigos HTTP correctos: 201 para recursos creados, 204 para
        eliminaciones exitosas sin cuerpo, 404 para no encontrado, 422 para
        validación fallida.
      </BlogCallout>
    </>
  );
...
function SectionSeguridad() {
  return (
    <>
      <BlogH2>Seguridad</BlogH2>
      <BlogH3>CORS y Helmet</BlogH3>
...
app.use(cors(corsOptions));`}</BlogCode>
      <BlogH3>Autenticación con JWT</BlogH3>
...
function verificarJWT(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    res.status(401).json({ error: "Token inválido o expirado" });
      ...
      <BlogH3>Validación con Zod</BlogH3>
...
router.post("/", validar(CrearTareaSchema), ctrl.crear);`}</BlogCode>
      <BlogCallout type="danger">
        Nunca almacenes contraseñas en texto plano. Usa siempre bcrypt o argon2
        con al menos 10 rondas de salt.
      </BlogCallout>
    </>
  );
}
...
function SectionArquitectura() {
  return (
    <>
      <BlogH2>Arquitectura: Controllers / Services / Repositories</BlogH2>
      <BlogP>
        Separar la lógica en capas hace el código más testeable y mantenible. El
        controller maneja HTTP, el service contiene la lógica de negocio, y el
        repository abstrae el acceso a datos.
      </BlogP>
      <BlogCode>{`// Estructura de carpetas
src/
...
└── index.ts         # Punto de entrada`}</BlogCode>
      <BlogCode>{`// services/tareas.service.ts
...
function SectionProyecto() {
  return (
    <>
      <BlogH2>Proyecto Final: REST API con autenticación</BlogH2>
      <BlogP>
        API completa para gestión de tareas con registro, login con JWT y
        operaciones CRUD protegidas.
      </BlogP>
      <BlogCode>{`// src/index.ts - Punto de entrada completo
import express    from "express";
...
const PORT = process.env.PORT ?? 3000;
app.listen(PORT, () => console.log(\`API en http://localhost:\${PORT}\`));`}</BlogCode>
...
export { router as authRouter };`}</BlogCode>
      <BlogCode>{`# Variables de entorno (.env)
PORT=3000
...
curl http://localhost:3000/api/tareas \\
  -H "Authorization: Bearer <token>"`}</BlogCode>
      <BlogCallout type="tip">
        Usa <code>prisma</code> como ORM para TypeScript. Ofrece migraciones,
        type-safety y una DX excelente con cualquier base de datos SQL o
        MongoDB.
      </BlogCallout>
    </>
  );
...
function SEjercicios() {
  return (
    <>
      <BlogH2>Ejercicios de Node.js + Express</BlogH2>
      <BlogP>
        Practica construyendo APIs reales con estos ejercicios. Asegúrate de
        tener Node.js instalado antes de comenzar.
      </BlogP>
...
app.listen(PORT, () => console.log(\`Servidor en http://localhost:\${PORT}\`));`}
          title="Servidor con rutas básicas"
        />
...
app.use((err, req, res, next) => {
  ...
  res.status(500).json({ error: "Error interno del servidor" });
          ...
          title="API con validación Zod y manejo de errores global"
        />
      </div>
    </>
  );
