...
export default function ContextSummarizerContent() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const data = await apiFetch<ToolResult>("/tools/summarize-context", {
        method: "POST",
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-[#1d1d1f] dark:text-white mb-2">Context Summarizer</h1>
        <p className="text-[#6e6e73] dark:text-[#86868b] text-sm">
          Extrae las frases más relevantes de un texto largo usando scoring de frecuencia.
          Útil para comprimir el historial de conversación o documentos antes de enviarlo a la IA.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <textarea
            ...
            placeholder="Pega aquí el texto largo o la conversación que quieres resumir…"
            rows={8}
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-sm text-fuchsia-500 font-semibold">{Math.round(targetRatio * 100)}%</span>
          </div>
          <input
            type="range"
            min={0.1}
          ...
          />
          <div className="flex justify-between text-xs text-[#aeaeb2] dark:text-[#636366] mt-1">
            <span>10% (muy comprimido)</span>
            <span>80% (ligero)</span>
          </div>
        </div>
        ...
        >
          {loading ? "Resumiendo…" : "Resumir contexto"}
        </button>
      </form>
      ...
      {result && (
        <div className="mt-8 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-sm font-medium text-[#1d1d1f] dark:text-white">Resumen</span>
              <button
