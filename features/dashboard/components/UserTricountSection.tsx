...
function computeBalances(group: Group): {
  ...
  for (const expense of group.expenses) {
    ...
    if (ledger[expense.paidBy] !== undefined)
  ...
  const balances: BalanceEntry[] = group.members.map((m) => ({
    member: m,
    net: ledger[m]!,
  }));
...
function mapApiExpense(e: TricountExpense, members: TricountMember[]): Expense {
  ...
  const splitNames = (e.splits ?? []).map(
    (s) => memberById[s.member_id] ?? s.member_id,
  );
...
function Avatar({ name, size = "sm" }: { name: string; size?: "sm" | "md" }) {
  ...
  return (
    <span
      className={`${sz} ${avatarColor(name)} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0`}
    >
      {initials(name)}
    </span>
  );
                    ...
                    filteredFriends.map((f) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-[10px] font-bold flex-shrink-0 ${avatarColor(label)}`}
                          >
                            {initials(label)}
                          </span>
...
function AddExpenseForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!desc.trim() || isNaN(amt) || amt <= 0 || splitWith.length === 0)
      return;
    setLoading(true);
    setError(null);
    try {
        ...
        <div className="flex flex-wrap gap-2">
          {members.map((m) => {
            ...
            return (
              ...
              >
                <span
                  className={`w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold ${avatarColor(m)} text-white`}
                >
  ...
  return (
    <div className="w-full rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116] flex flex-col">
      <div className="flex items-center">
        ...
        >
          <div
            className={`w-11 h-11 rounded-2xl flex items-center justify-center text-white font-bold text-lg flex-shrink-0 ${avatarColor(group.name)}`}
          >
            {group.name[0]}
          </div>
          <div className="flex-1 min-w-0">
...
function AddMemberModal({
  ...
  const existingUserIds = new Set(
    group.memberObjects.map((m) => m.user_id).filter(Boolean),
  );
            ...
            available.map((f) => {
              ...
              return (
                <div key={f.id} className="flex items-center gap-3 px-3 py-2.5">
                  <span
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${avatarColor(label)}`}
                  >
                    {initials(label)}
                  </span>
                  <div className="flex-1 min-w-0">
            ...
            return (
              ...
              >
                <div className="p-4 flex items-center gap-3">
                  <div
                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-white flex-shrink-0 ${avatarColor(expense.desc)}`}
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
...
export function UserTricountSection() {
  ...
  async function handleSelectGroup(groupId: string) {
    ...
    if (
      existing &&
      existing.expensesLoaded &&
      existing.memberObjects.length > 0
    )
      return;
