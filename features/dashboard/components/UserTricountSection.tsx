...
import { mobileService, type MobileAppVersion, type MobileLatest } from "../../../services/mobileService";
import {
	tricountService,
...
function AppDownloadCard() {
  ...
  const [allVersions, setAllVersions] = useState<MobileAppVersion[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    Promise.all([
      mobileService.getLatest().catch(() => null),
      mobileService.listVersions("android").catch(() => []),
    ]).then(([lat, versions]) => {
      setLatest(lat);
      // Sort by semver descending, exclude duplicates of latest (latest-files)
      const sorted = [...versions].sort((a, b) => semverCompare(a.version, b.version));
      setAllVersions(sorted);
    }).finally(() => setLoading(false));
  }, []);

  async function handleDownloadLatest(buildType: "apk" | "aab" | "ipa") {
  ...
  async function handleDownloadVersion(version: MobileAppVersion) {
    ...
    setError(null);
    try {
      await mobileService.downloadById(version);
    } catch {
  ...
  return (
    <div className="rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116] overflow-hidden">
      ...
      <div className="p-5 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-[#1d1d1f] dark:text-white">Saldos</p>
      ...
      <div className="px-5 pb-4 flex flex-wrap gap-2">
        {/* Android APK */}
        <button
          onClick={() => handleDownloadLatest("apk")}
          disabled={!hasAndroid || downloading === "apk" || loading}
        ...
        >
          ...
          Android
          {apk?.file_size && (
            <span className="text-white/70 text-[11px]">({formatBytes(apk.file_size)})</span>
          )}
        </button>

        {/* iOS — always shown, always disabled */}
        <button
          disabled
          className="flex items-center gap-2 bg-[#007aff]/40 cursor-not-allowed text-white/70 text-sm font-medium px-4 py-2 rounded-xl relative group"
          title="iOS próximamente"
        >
          <IcoApple />
          iOS
          <span className="ml-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-white/20">Próximamente</span>
        </button>
      </div>
      ...
      {!loading && allVersions.length > 0 && (
        <div className="border-t border-black/5 dark:border-white/5">
          ...
          >
            <span>Historial de versiones ({allVersions.length})</span>
            <svg
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${
                          v.build_type === "apk"
                            ? "bg-[#34c759]/10 text-[#30b350]"
                            : "bg-sky-50 dark:bg-sky-950/30 text-sky-600 dark:text-sky-400"
                        }`}>
                          {v.build_type.toUpperCase()}
                        </span>
                        {isActive && (
                          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/30 text-indigo-600 dark:text-indigo-400">
                            Activa
                          </span>
                        )}
                      </div>
