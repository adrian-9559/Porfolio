...
function AppCard({ app, latest }: AppCardProps) {
  ...
  async function toggleHistory(system?: "android" | "ios") {
    if (!showHistory && versions.length === 0) {
      ...
      try {
        const all = await mobileAppService.listVersions(
          app.slug,
          system || "android",
        );
...
export function UserAppsSection() {
  ...
  useEffect(() => {
    Promise.all([mobileAppService.listApps(), mobileAppService.getLatest()])
      .then(([appList, latest]) => {
        setApps(appList);
        setLatestMap(latest);
      })
