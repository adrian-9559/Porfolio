		...
		<div className="space-y-6">
			...
			<div className="flex items-center gap-4">
				...
				<div>
					...
					<p className="text-xs text-[#aeaeb2] dark:text-[#636366] mt-0.5">JPG, PNG o GIF. Máx 2MB</p>
				</div>
			</div>

			<div className="space-y-4">
				<div>
					<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-2">Nombre</label>
				...
				<div>
					<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-2">Email</label>
					...
					<p className="text-xs text-[#aeaeb2] mt-1">El email no se puede cambiar</p>
				</div>

				<div>
					<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-2">Bio</label>
					<textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} placeholder="Cuéntanos algo sobre ti..."
				...
				<div>
					<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-2">Sitio web</label>
					<input value={website} onChange={(e) => setWebsite(e.target.value)} placeholder="https://tuportfolio.com" type="url"
						className="w-full px-3 py-2 rounded-xl border border-black/12 dark:border-white/12 bg-black/3 dark:bg-white/5 text-sm text-[#1d1d1f] dark:text-white placeholder:text-[#aeaeb2] focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 transition-all" />
				</div>
			</div>
...
function SecurityTab() {
	...
	function changePassword() {
		...
		if (newPwd.length < 8) return setMsg("La contraseña debe tener al menos 8 caracteres");
	...
	return (
		<div className="space-y-8">
			{/* Password */}
			<div>
				<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white mb-1">Cambiar contraseña</p>
				<p className="text-xs text-[#6e6e73] dark:text-[#86868b] mb-4">Usa una contraseña segura de al menos 8 caracteres</p>
				...
				<div className="space-y-3">
					{[
						...
						<div key={f.label}>
							<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-1.5">{f.label}</label>
			...
			{/* Active sessions */}
			<div>
				<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white mb-1">Sesiones activas</p>
					...
					{SESSIONS.map((s, i) => (
						<div key={i} className="flex items-center gap-4 px-4 py-3">
							<div className="w-8 h-8 rounded-lg bg-black/5 dark:bg-white/5 flex items-center justify-center flex-shrink-0">
								<svg className="w-4 h-4 text-[#6e6e73] dark:text-[#86868b]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
							...
							{!s.current && (
								<button className="text-xs text-red-500 hover:text-red-600 hover:underline">Cerrar</button>
							)}
						</div>
					))}
				</div>
...
function SessionTab() {
	...
	function save() {
		setSaved(true);
		try { localStorage.setItem("user-prefs", JSON.stringify({ rememberMe, expiry, theme })); } catch {}
		setTimeout(() => setSaved(false), 2000);
	}
	...
	return (
		<div className="space-y-8">
			...
			<div className="rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116] p-5 space-y-4">
				<div>
					<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white mb-0.5">Recordar sesión</p>
					<p className="text-xs text-[#6e6e73] dark:text-[#86868b]">Mantén la sesión activa entre cierres de navegador</p>
				</div>

				<div className="flex items-center justify-between py-3 border-t border-black/5 dark:border-white/5">
					<div>
						<p className="text-sm font-medium text-[#1d1d1f] dark:text-white">Recordar inicio de sesión</p>
						<p className="text-xs text-[#aeaeb2] dark:text-[#636366]">
							{rememberMe ? "Sesión persistente activada" : "La sesión expira al cerrar el navegador"}
						</p>
					</div>
					<button onClick={() => setRememberMe((v) => !v)}
						...
						<span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${rememberMe ? "translate-x-5" : "translate-x-1"}`} />
					</button>
				</div>

				{rememberMe && (
					<div>
						<label className="text-[10px] font-semibold uppercase tracking-wider text-[#aeaeb2] dark:text-[#636366] block mb-2">Duración de la sesión</label>
							...
							{[
								...
								<button key={opt.id} onClick={() => setExpiry(opt.id)}
									className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
										expiry === opt.id ? "bg-blue-600 text-white" : "border border-black/12 dark:border-white/12 text-[#6e6e73] dark:text-[#86868b] hover:bg-black/5 dark:hover:bg-white/5"
									}`}>
									{opt.label}
								</button>
							))}
			...
			<div className="rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116] p-5 space-y-4">
				<div>
					<p className="text-sm font-semibold text-[#1d1d1f] dark:text-white mb-0.5">Apariencia</p>
					<p className="text-xs text-[#6e6e73] dark:text-[#86868b]">Personaliza el tema de la interfaz</p>
				</div>
					...
					{[
						...
						<button key={opt.id} onClick={() => setTheme(opt.id)}
							className={`p-3 rounded-xl border text-center transition-all ${
								theme === opt.id ? "border-blue-500 bg-blue-50 dark:bg-blue-950/20" : "border-black/12 dark:border-white/12 hover:bg-black/3 dark:hover:bg-white/3"
							}`}>
							<span className="text-xl">{opt.icon}</span>
							<p className="text-xs font-medium text-[#1d1d1f] dark:text-white mt-1">{opt.label}</p>
						</button>
					))}
				</div>
			</div>
...
export function UserSettingsSection({ tab, onTabChange }: Props) {
	return (
		<div className="space-y-6">
			<div>
				<h2 className="text-lg font-semibold text-[#1d1d1f] dark:text-white">Configuración</h2>
				<p className="text-sm text-[#6e6e73] dark:text-[#86868b] mt-0.5">Gestiona tu perfil y preferencias de cuenta</p>
			</div>
