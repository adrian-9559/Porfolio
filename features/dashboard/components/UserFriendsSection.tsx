...
function FriendsTab({
  ...
  if (friends.length === 0)
    return <EmptyState text="No tienes amigos aún. ¡Busca a alguien!" />;
...
function ReceivedTab({
  ...
  if (requests.length === 0)
    return <EmptyState text="No tienes solicitudes pendientes." />;
...
function SentTab({
  ...
  if (loading) return <Spinner />;
  if (requests.length === 0)
    return <EmptyState text="No has enviado solicitudes." />;
  ...
  return (
    <div className="flex flex-col gap-2">
      {requests.map((req) => {
        ...
        const name =
          receiver?.full_name ?? receiver?.email ?? req.receiver_user_id;
...
function SearchTab({ onRequestSent }: { onRequestSent: () => void }) {
  ...
  async function handleSendRequest() {
    ...
    try {
      ...
      setSent(true);
      setResult((prev) =>
        prev ? { ...prev, relationship: "pending_sent" } : prev,
      );
      onRequestSent();
    } catch (err: any) {
...
export function UserFriendsSection() {
  ...
  const [tabLoaded, setTabLoaded] = useState<Set<Tab>>(
    new Set(["amigos", "recibidas", "enviadas"]),
  );
  ...
  const metrics = [
    ...
    {
      label: "Recibidas",
      value: loadingReceived ? "…" : String(received.length),
    },
  ...
  return (
    <section className="flex flex-col gap-4">
      ...
      {tab === "enviadas" && <SentTab loading={loadingSent} requests={sent} />}
      {tab === "buscar" && <SearchTab onRequestSent={() => fetchAll()} />}
    </section>
  );
}
