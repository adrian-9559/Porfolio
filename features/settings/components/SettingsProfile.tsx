...
export function SettingsProfile() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Card title="Perfil">
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-600 to-cyan-500 flex items-center justify-center text-white text-xl font-semibold">{initials}</div>
            <div>
              <p className="text-sm font-medium text-[#1d1d1f] dark:text-white">{user?.email}</p>
              <div className="flex gap-1.5 mt-1 flex-wrap">
                {user?.roles.map((r) => (
                  <span key={r} className="px-2 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 text-xs font-medium">{r}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-medium text-[#1d1d1f] dark:text-white">Biografía</label>
            <textarea value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Cuéntanos algo sobre ti…" rows={3} maxLength={500}
          ...
          <div className="flex items-center gap-3 mt-1">
            <button type="submit" disabled={saving} className="px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white text-sm font-medium transition-colors">
      ...
      <Card title="Cuenta">
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between py-1">
            <div>
              <p className="text-sm font-medium text-[#1d1d1f] dark:text-white">Email</p>
          ...
          <div className="border-t border-black/6 dark:border-white/6 pt-3">
            <p className="text-xs text-[#6e6e73] dark:text-[#86868b] mb-3">Para cambiar tu contraseña, recibirás un email con instrucciones.</p>
      ...
      <Card title="Sesión">
        <button onClick={logout} className="px-4 py-2 rounded-xl border border-red-200 dark:border-red-800 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
          Cerrar sesión
        </button>
      </Card>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116] overflow-hidden">
      <div className="px-6 py-4 border-b border-black/6 dark:border-white/6">
        <h2 className="text-sm font-semibold text-[#1d1d1f] dark:text-white">{title}</h2>
      </div>
