    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 origin-top-right z-50 animate-in fade-in zoom-in-95 duration-150">
          <div className="ds-modal-content !rounded-2xl">
            {/* User info header */}
            <div
            ...
            >
              ...
              <div className="relative flex items-center gap-1.5 mt-3">
                ...
                <LanguageSwitcher />
                {isAdmin && (
                  <span className="ml-auto ds-badge ds-badge-accent">
                    ADMIN
                  </span>
                )}
              </div>
...
function MenuItem({
  ...
  return (
    <button
      ...
      style={
        !danger && !accent ? { color: "var(--text-secondary)" } : undefined
      }
      type="button"
      onClick={onClick}
    >
      <span
        className={`${danger ? "text-red-500 dark:text-red-400" : accent ? "text-blue-500 dark:text-blue-400" : ""} flex-shrink-0`}
        style={!danger && !accent ? { color: "var(--text-muted)" } : undefined}
      >
        {icon}
      </span>
      {label}
