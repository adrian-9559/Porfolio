            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-black/8 dark:border-white/8">
                  <th className="text-left text-xs font-semibold text-[#6e6e73] dark:text-[#86868b] p-4">
                    {t("admin.trafficPagePath")}
                  </th>
                  <th className="text-right text-xs font-semibold text-[#6e6e73] dark:text-[#86868b] p-4">
              ...
              <tbody>
                {trafficByPage.map((page, i) => (
                  <tr
                    key={i}
                    className="border-b border-black/4 dark:border-white/4 hover:bg-black/[0.02] dark:hover:bg-white/[0.02]"
                  >
                    <td className="p-4">
                    ...
                    <td className="p-4 text-right text-sm font-semibold text-[#1d1d1f] dark:text-white">
                      {page.visits}
                    </td>
                    <td className="p-4 text-right text-sm text-[#6e6e73] dark:text-[#86868b]">
                      {page.uniqueVisitors}
                    </td>
                  </tr>
                ))}
              ...
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-semibold text-[#1d1d1f] dark:text-white">
                    {t("admin.trafficVisitorHistory")} —{" "}
                    <code className="text-xs">{selectedVisitor}</code>
                  </h3>
                  <button
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {visitorHistory.map((h, i) => (
                    <div key={i} className="flex items-center gap-3 text-xs">
                      <span className="text-[#aeaeb2] dark:text-[#636366] w-20 flex-shrink-0">
                        {formatDate(h.created_at)}
                      </span>
                      <code className="font-mono text-[#1d1d1f] dark:text-white bg-black/5 dark:bg-white/10 px-2 py-0.5 rounded">
                        {h.page_path}
                      </code>
                      {h.duration_ms && (
                ...
                <tbody>
                  {visitors.map((v, i) => (
                    ...
                    >
                      ...
                      <td className="p-4 text-xs text-[#6e6e73] dark:text-[#86868b]">
                      ...
                      <td className="p-4">
                        <div className="flex flex-wrap gap-1">
                          {v.pages.slice(0, 3).map((p: string, j: number) => (
                            <span
                              key={j}
                              className="px-1.5 py-0.5 rounded text-[10px] bg-black/5 dark:bg-white/10 text-[#6e6e73] dark:text-[#86868b]"
                            >
                              {p}
                            </span>
                          ))}
            ...
            <table className="w-full">
              ...
              <tbody>
                {flow.map((f, i) => (
                  ...
                  >
                    <td className="p-4">
                      <code className="text-xs font-mono text-[#1d1d1f] dark:text-white">
                        {f.flow}
                      </code>
                    </td>
                    <td className="p-4 text-right text-sm font-semibold text-[#1d1d1f] dark:text-white">
