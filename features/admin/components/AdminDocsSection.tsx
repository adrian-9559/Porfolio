...
const ENDPOINTS: Endpoint[] = [
  ...
  // Agents
  { method: "GET",    path: "/api/tools/ai-agents/presets",           summary: "Get agent presets",          auth: true,                          module: "agents" },
  { method: "POST",   path: "/api/tools/ai-agents/from-preset",       summary: "Create agent from preset",   auth: true,                          module: "agents" },
  { method: "POST",   path: "/api/tools/ai-agents/create",            summary: "Create custom agent",        auth: true,                          module: "agents" },
  { method: "GET",    path: "/api/tools/ai-agents",                   summary: "List own agents",            auth: true,                          module: "agents" },
  { method: "GET",    path: "/api/tools/ai-agents/:id",               summary: "Get agent by ID",            auth: true,                          module: "agents" },
  { method: "PUT",    path: "/api/tools/ai-agents/:id",               summary: "Update agent",               auth: true,                          module: "agents" },
  { method: "DELETE", path: "/api/tools/ai-agents/:id",               summary: "Delete agent",               auth: true,                          module: "agents" },
  { method: "POST",   path: "/api/tools/ai-agents/run",               summary: "Execute agent",              auth: true, rateLimit: "20/min",     module: "agents" },
  { method: "POST",   path: "/api/tools/ai-agents/orchestrate",       summary: "Orchestrate multiple agents",auth: true, rateLimit: "20/min",     module: "agents" },
  { method: "GET",    path: "/api/tools/ai-agents/:id/memory",        summary: "Get agent memory",           auth: true,                          module: "agents" },
  { method: "DELETE", path: "/api/tools/ai-agents/:id/memory",        summary: "Clear agent memory",         auth: true,                          module: "agents" },
  // Workflows
  { method: "POST",   path: "/api/tools/ai-agents/workflows/create",  summary: "Create workflow",            auth: true,                          module: "workflows" },
  { method: "GET",    path: "/api/tools/ai-agents/workflows",          summary: "List workflows",             auth: true,                          module: "workflows" },
  { method: "GET",    path: "/api/tools/ai-agents/workflows/:id",      summary: "Get workflow",               auth: true,                          module: "workflows" },
  { method: "POST",   path: "/api/tools/ai-agents/workflows/run",      summary: "Execute workflow",           auth: true, rateLimit: "20/min",    module: "workflows" },
  { method: "DELETE", path: "/api/tools/ai-agents/workflows/:id",      summary: "Delete workflow",            auth: true,                          module: "workflows" },
  // Graphs
  { method: "POST",   path: "/api/tools/ai-agents/graphs/create",     summary: "Save graph layout",          auth: true,                          module: "graphs" },
  { method: "GET",    path: "/api/tools/ai-agents/graphs",            summary: "List graphs",                auth: true,                          module: "graphs" },
  { method: "GET",    path: "/api/tools/ai-agents/graphs/:id",        summary: "Get graph",                  auth: true,                          module: "graphs" },
  { method: "PUT",    path: "/api/tools/ai-agents/graphs/:id",        summary: "Update graph",               auth: true,                          module: "graphs" },
  { method: "DELETE", path: "/api/tools/ai-agents/graphs/:id",        summary: "Delete graph",               auth: true,                          module: "graphs" },
  // Repositories
  ...
  { method: "GET",    path: "/admin/repositories",                    summary: "All repositories",           auth: true, roles: ["admin"],         module: "admin" },
  { method: "DELETE", path: "/admin/repositories/:id",                summary: "Delete any repository",      auth: true, roles: ["admin"],         module: "admin" },
];

const MODULES = ["all", "auth", "users", "roles", "api-keys", "notifications", "contact", "tools", "agents", "workflows", "graphs", "repositories", "admin"] as const;
...
export function AdminDocsSection() {
  ...
  const tabCls = (v: View) =>
    `px-4 py-2 rounded-xl text-sm font-medium transition-colors ${view === v
      ? "bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f]"
      : "text-[#6e6e73] dark:text-[#86868b] hover:bg-black/5 dark:hover:bg-white/5"}`;

  return (
    <div className="flex flex-col gap-5">
      <SectionHeader title="Documentación" desc={`${ENDPOINTS.length} endpoints · referencia técnica completa`} />
      ...
      {view === "endpoints" && (
        <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-1 flex-wrap">
              {MODULES.map((m) => (
                <button
                  ...
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-colors ${activeModule === m
          ...
          <p className="text-xs text-[#aeaeb2] dark:text-[#636366]">{filtered.length} endpoint{filtered.length !== 1 ? "s" : ""}</p>

          {/* Endpoint list */}
          <div className="bg-white dark:bg-[#1c1c1e] border border-black/[0.06] dark:border-white/[0.06] rounded-2xl overflow-hidden">
            {filtered.length === 0 ? (
              <p className="px-5 py-8 text-sm text-center text-[#6e6e73] dark:text-[#86868b]">No endpoints match</p>
            ) : (
              <div className="divide-y divide-black/[0.04] dark:divide-white/[0.04]">
                {filtered.map((e, i) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold font-mono w-14 text-center flex-shrink-0 ${METHOD_COLOR[e.method]}`}>
                      {e.method}
                    </span>
                    <code className="text-xs font-mono text-[#1d1d1f] dark:text-white flex-1 truncate">{e.path}</code>
                    <p className="text-xs text-[#6e6e73] dark:text-[#86868b] flex-1 min-w-0 truncate hidden sm:block">{e.summary}</p>
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {e.auth ? (
                        <span className="px-1.5 py-0.5 rounded text-[9px] bg-amber-50 dark:bg-amber-950/30 text-amber-600 font-medium">auth</span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded text-[9px] bg-green-50 dark:bg-green-950/30 text-green-600 font-medium">public</span>
                      )}
                      {e.roles?.map((r) => (
                        <span key={r} className="px-1.5 py-0.5 rounded text-[9px] bg-purple-50 dark:bg-purple-950/30 text-purple-600 font-medium">{r}</span>
                      ))}
                      {e.rateLimit && (
                        <span className="px-1.5 py-0.5 rounded text-[9px] bg-black/[0.04] dark:bg-white/[0.04] text-[#6e6e73] dark:text-[#86868b] font-mono">{e.rateLimit}</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          <div className="flex gap-1 flex-wrap">
            {docs.map((d) => (
              <button key={d.id} onClick={() => setActiveDoc(d.id)}
                className={`px-3 py-1.5 rounded-xl text-xs font-medium transition-colors ${activeDoc === d.id
                  ? "bg-blue-600 text-white"
                  : "border border-black/10 dark:border-white/10 text-[#6e6e73] dark:text-[#86868b] hover:bg-black/5 dark:hover:bg-white/5"
                }`}>
                {d.title}
          ...
          <div className="flex flex-col gap-4">
            {currentDoc.content.map((section) => (
              <Card key={section.heading} className="px-5 py-4">
                <h3 className="text-sm font-semibold text-[#1d1d1f] dark:text-white mb-3">{section.heading}</h3>
                <ul className="space-y-2">
                  {section.items.map((item, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#6e6e73] dark:text-[#86868b]">
