...
function LogIcon({ icon, gradient }: { icon: string; gradient: string }) {
  ...
  return (
    <div
      className={`w-9 h-9 rounded-xl bg-gradient-to-br ${gradient} flex items-center justify-center text-white shadow-md shrink-0 mt-0.5`}
    >
      {svg}
    </div>
  );
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#6e6e73] dark:text-[#86868b] mt-1">
            {events.length} eventos · Actualizado{" "}
          ...
          <div className="divide-y divide-black/5 dark:divide-white/5">
            {filtered.map((event) => {
              ...
              return (
                ...
                >
                  {/* Icon */}
                  <LogIcon gradient={cfg.gradient} icon={event.icon} />
                  {/* Content */}
