...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-semibold capitalize ${colors[status] ?? colors.rejected}`}
    >
      {status}
    </span>
  );
    ...
    <div className="relative flex flex-col gap-6 overflow-hidden">
      ...
      {tab === "friendships" && (
        <div className="relative overflow-hidden rounded-2xl border border-black/8 dark:border-white/8 bg-white dark:bg-[#111116]">
          <div
            className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-emerald-500 to-green-500 opacity-0 transition-opacity hover:opacity-100"
            ...
            <>
              ...
              <div className="divide-y divide-black/5 dark:divide-white/5">
                {friendships.map((f) => (
                  <div
                    key={f.id}
                    className="group/item relative grid grid-cols-3 gap-4 px-5 py-3.5 items-center transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02]"
                  >
                    ...
                    <p className="truncate text-sm text-[#1d1d1f] dark:text-white">
                      {f.user_a_email}
                    </p>
                    <p className="truncate text-sm text-[#1d1d1f] dark:text-white">
                      {f.user_b_email}
            ...
            <>
              ...
              <div className="divide-y divide-black/5 dark:divide-white/5">
                {requests.map((r) => (
                  <div
                    key={r.id}
                    className="group/item relative grid grid-cols-4 gap-4 px-5 py-3.5 items-center transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.02]"
                  >
                    <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-emerald-500/40 to-transparent opacity-0 transition-opacity group-hover/item:opacity-100" />
                    <p className="truncate text-sm text-[#1d1d1f] dark:text-white">
                      {r.sender_email}
