...
export function AdminDashboard({
  ...
  const maxDayVisits = useMemo(
    () => Math.max(...trafficByDay.map((d) => d.visits), 1),
    [trafficByDay],
  );
  const maxPageVisits = useMemo(
    () => Math.max(...topPages.map((p) => p.visits), 1),
    [topPages],
  );
        ...
        <>
          ...
          <div className="backdrop-blur-xl rounded-2xl p-2 bg-white/60 dark:bg-[#111116]/60 border border-black/8 dark:border-white/8">
            <div className="flex gap-2 overflow-x-auto pb-1">
              {SHORTCUTS.map((sc) => (
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl bg-gradient-to-br ${sc.iconBg} flex items-center justify-center text-white shadow-lg`}
                  >
                    <ShortcutIcon id={sc.id} />
                  </div>
                  <span className="text-xs font-semibold text-[#1d1d1f] dark:text-white truncate max-w-[68px]">
                    {t(SHORTCUT_LABELS[sc.id])}
                  </span>
                </button>
              ))}
            </div>
          ...
          {stats && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
              {[
                ...
                >
                  <div
                    className={`h-1 bg-gradient-to-r ${m.gradient} opacity-80 group-hover:opacity-100 transition-opacity`}
                  />
                  <div className="p-4 relative">
                    <div
                      className={`absolute -bottom-5 -right-5 w-20 h-20 rounded-full bg-gradient-to-br ${m.gradient} opacity-10 blur-2xl`}
                    />
                    <div className="flex items-center gap-2.5">
                      <div
                        className={`w-9 h-9 rounded-xl bg-gradient-to-br ${m.iconBg} flex items-center justify-center text-white shadow-lg shrink-0`}
                      >
                    ...
                    {m.sub && (
                      <p className="text-[10px] text-[#6e6e73] dark:text-[#86868b] mt-1.5">
                        {m.sub}
                      </p>
                    )}
                  </div>
            ...
            {[
              ...
              >
                ...
                <div className="p-4 relative">
                  ...
                  <div className="flex items-center gap-2.5">
                    <div
                      className={`w-9 h-9 rounded-xl bg-gradient-to-br ${m.gradient} flex items-center justify-center text-white shadow-lg shrink-0`}
                    >
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-semibold text-[#1d1d1f] dark:text-white">
                        {m.label}
                      </p>
                    </div>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-black/20">
              ...
              <div className="p-5">
                <h3
                  className="text-sm font-bold tracking-tight text-[#1d1d1f] dark:text-white mb-4"
                  style={{ letterSpacing: "-0.02em" }}
                >
                  {t("admin.trafficChart")}
                </h3>
                {trafficByDay.length === 0 ? (
                  <div className="h-32 flex items-center justify-center text-xs text-[#6e6e73] dark:text-[#86868b]">
                  ...
                  <div className="flex flex-col gap-2.5">
                    {topPages.map((p, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <span
                          className="text-xs text-[#6e6e73] dark:text-[#86868b] w-28 truncate shrink-0 font-medium"
                          title={p.path}
                        >
                          {p.path}
                        </span>
                        <div className="flex-1 h-5 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${(p.visits / maxPageVisits) * 100}%`,
                            }}
                          />
                        </div>
                        <span className="text-xs font-bold text-[#1d1d1f] dark:text-white tabular-nums w-8 text-right">
                          {p.visits}
                        </span>
                      </div>
                    ))}
            ...
            <div className="rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-black/20">
              ...
              <div className="p-5">
                ...
                {issueStats && Object.keys(issueStats.byStatus).length > 0 ? (
                  <div className="flex items-center gap-5">
                    <div
                      ...
                      style={{
                        ...
                        boxShadow:
                          "inset 0 0 0 10px var(--heroui-background, #fff), 0 4px 20px rgba(0,0,0,0.1)",
                      }}
                    />
                        ...
                        ([status, count]) => (
                          ...
                          >
                            <span
                              ...
                              style={{
                                background: issueStatusColors[status] ?? "#888",
                              }}
                            />
                            <span className="text-[#6e6e73] dark:text-[#86868b] font-medium">
                              {status}
                            </span>
                            <span className="font-bold text-[#1d1d1f] dark:text-white ml-auto tabular-nums">
                              {count}
                            </span>
                          </div>
                        ),
                  ...
                  <div className="h-28 flex items-center justify-center text-xs text-[#6e6e73] dark:text-[#86868b]">
                    {t("admin.noTickets")}
                  </div>
                )}
              </div>
              ...
              <div className="p-5">
                ...
                {ideaStats && Object.keys(ideaStats.byStatus).length > 0 ? (
                  <div className="flex items-center gap-5">
                    ...
                    />
                    <div className="flex flex-col gap-1.5">
                      {Object.entries(ideaStats.byStatus).map(
                        ([status, count]) => (
                          <div
                            key={status}
                            className="flex items-center gap-2.5 text-xs"
                          >
                            <span
                              className="w-3 h-3 rounded-full shrink-0 shadow-sm"
                              style={{
                                background: ideaStatusColors[status] ?? "#888",
                              }}
                            />
                            <span className="text-[#6e6e73] dark:text-[#86868b] font-medium">
                              {status.replace("_", " ")}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-black/20">
              ...
              <div className="px-5 py-4 border-b border-black/6 dark:border-white/6">
                <h3
                  className="text-sm font-bold tracking-tight text-[#1d1d1f] dark:text-white"
                  style={{ letterSpacing: "-0.02em" }}
                >
              ...
              {activityFeed.length === 0 ? (
                <div className="px-5 py-10 text-center text-sm text-[#6e6e73] dark:text-[#86868b]">
                  {t("admin.noActivity")}
                </div>
              ) : (
                <div className="divide-y divide-black/5 dark:divide-white/5 max-h-96 overflow-y-auto">
                  {activityFeed.map((item, i) => (
                    <div
                      key={i}
                      className="flex items-start gap-3 px-5 py-3 hover:bg-black/2 dark:hover:bg-white/2 transition-colors"
                    >
                      ...
                      <span className="text-[10px] text-[#aeaeb2] dark:text-[#636366] shrink-0 font-medium">
            ...
            <div className="rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-black/20">
              ...
              <div className="px-5 py-4 border-b border-black/6 dark:border-white/6 flex items-center justify-between">
                ...
                {health && (
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold text-white shadow-lg ${health.status === "ok" && health.db?.ok !== false ? "bg-gradient-to-r from-emerald-500 to-teal-500" : "bg-gradient-to-r from-red-500 to-rose-500"}`}
                  >
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        ...
                        {
                          label: t("admin.healthDbChecked"),
                          value: health.db
                            ? relativeTime(health.db.checkedAt)
                            : "—",
                        },
                        {
                    ...
                    {health.db?.tables && (
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {Object.entries(health.db.tables).map(
                            ([table, ok]) => (
                              <span
                                key={table}
          ...
          <div className="rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-black/5 dark:hover:shadow-black/20">
            ...
            <div className="px-5 py-4 border-b border-black/6 dark:border-white/6 flex items-center justify-between">
              ...
              <span className="text-xs text-[#aeaeb2] dark:text-[#636366] font-medium bg-black/5 dark:bg-white/10 px-2.5 py-1 rounded-full">
            ...
            <div className="divide-y divide-black/5 dark:divide-white/5">
              {recentUsers.length === 0 ? (
                ...
                recentUsers.map((u) => (
                  <div
                    key={u.id}
                    className="flex items-center gap-3 px-5 py-3.5 hover:bg-black/2 dark:hover:bg-white/2 transition-colors"
                  >
                    ...
                    <div className="flex gap-1 flex-wrap justify-end">
                      {u.roles.map((r) => (
                        <span
                          key={r.id}
                          className="px-2.5 py-0.5 rounded-lg bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-950/40 dark:to-indigo-950/30 text-blue-700 dark:text-blue-400 text-[10px] font-bold"
                        >
                          {r.name}
                        </span>
                      ))}
    ...
    case "ideas":
      return (
        <svg
          ...
          viewBox="0 0 24 24"
        >
          <path d="M12 2a7 7 0 00-2 13.7V18h4v-2.3A7 7 0 0012 2z" />
          <path d="M9 21h6" />
        </svg>
      );
    case "skills":
    ...
    case "ticket":
      return (
        <svg
          ...
          viewBox="0 0 24 24"
        >
          <path d="M15 5v2m0 4v2m0 4v2M5 5h14a2 2 0 012 2v3a2 2 0 000 4v3a2 2 0 01-2 2H5a2 2 0 01-2-2v-3a2 2 0 000-4V7a2 2 0 012-2z" />
        </svg>
      );
    case "lightbulb":
      return (
...
function ActivityIcon({ type }: { type: string }) {
  ...
  return (
    <div
      className={`w-9 h-9 rounded-xl bg-gradient-to-br ${gradients[type] ?? gradients.user} flex items-center justify-center text-white shadow-md shrink-0`}
    >
