...
import { mobileAppService, type App, type MobileAppVersion } from "@/services/mobileAppService";
...
function AppModal({ initial, onClose, onSaved }: AppModalProps) {
  ...
  const [accentColor, setAccentColor] = useState(initial?.accent_color ?? "#34c759");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!initial && !slug.trim()) { setError("El slug es obligatorio."); return; }
    setLoading(true);
    try {
      const payload = { name: name.trim(), description: description.trim() || undefined, icon_emoji: iconEmoji.trim() || undefined, accent_color: accentColor, is_published: isPublished };
      const app = initial
        ? await mobileAppService.updateApp(initial.id, payload)
        : await mobileAppService.createApp({ ...payload, slug: slug.trim() } as any);
      onSaved(app);
      onClose();
    } catch (e: unknown) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white dark:bg-[#111116] rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-black/8 dark:border-white/8">
          <h3 className="text-base font-semibold text-[#1d1d1f] dark:text-white">{initial ? "Editar app" : "Nueva app"}</h3>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          {!initial && (
            <div>
              <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Slug <span className="text-red-500">*</span></label>
              <input value={slug} onChange={(e) => setSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))} placeholder="mi-app"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Nombre <span className="text-red-500">*</span></label>
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nombre de la app"
                className="w-full rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white placeholder-[#aeaeb2] focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Emoji icono</label>
              <input value={iconEmoji} onChange={(e) => setIconEmoji(e.target.value)} placeholder="💸"
                className="w-full rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white placeholder-[#aeaeb2] focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Color acento</label>
              <div className="flex gap-2 items-center">
                <input type="color" value={accentColor} onChange={(e) => setAccentColor(e.target.value)} className="w-8 h-8 rounded-lg border border-black/12 dark:border-white/12 cursor-pointer" />
                <input value={accentColor} onChange={(e) => setAccentColor(e.target.value)} className="flex-1 rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
            </div>
          </div>
          <div>
            <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Descripción</label>
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm text-[#1d1d1f] dark:text-white">Publicada (visible para usuarios)</span>
          </label>
          {error && <p className="text-sm text-red-500">{error}</p>}
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={onClose} className="flex-1 py-2 rounded-xl border border-black/12 dark:border-white/12 text-sm text-[#6e6e73] hover:text-[#1d1d1f] dark:hover:text-white transition-colors">Cancelar</button>
            <button type="submit" disabled={loading} className="flex-1 py-2 rounded-xl bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] text-sm font-medium disabled:opacity-50 transition-opacity">
...
function UploadModal({ apps, preselectedAppId, onClose, onUploaded }: UploadModalProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!appId) { setError("Selecciona una app."); return; }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white dark:bg-[#111116] rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-4">
          <div>
            <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">App <span className="text-red-500">*</span></label>
            <select value={appId} onChange={(e) => setAppId(e.target.value)}
              ...
              {apps.map((a) => <option key={a.id} value={a.id}>{a.icon_emoji ? `${a.icon_emoji} ` : ""}{a.name}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              ...
              <select value={buildType} onChange={(e) => setBuildType(e.target.value)}
                className="w-full rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500">
                {BUILD_TYPES[platform].map((bt) => <option key={bt} value={bt}>{BUILD_LABELS[bt]}</option>)}
              </select>
            </div>
          </div>
          <div>
            ...
            <div className="flex gap-2">
              {(["url", "file"] as const).map((t) => (
                <button key={t} type="button" onClick={() => setSourceType(t)}
                  className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-colors ${sourceType === t ? "bg-blue-50 dark:bg-blue-950/30 border-blue-300 dark:border-blue-700 text-blue-600 dark:text-blue-400" : "border-black/12 dark:border-white/12 text-[#6e6e73] hover:text-[#1d1d1f] dark:hover:text-white"}`}>
          ...
          {sourceType === "url" ? (
            <div>
              ...
              <input value={externalUrl} onChange={(e) => setExternalUrl(e.target.value)}
                ...
                className="w-full rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white placeholder-[#aeaeb2] focus:outline-none focus:ring-2 focus:ring-blue-500" />
              {externalUrl.trim() && (
                <p className="mt-1.5 text-xs text-[#6e6e73] truncate">→ <span className="font-mono">{toRawGithubUrl(externalUrl)}</span></p>
              )}
            </div>
          ) : (
            <div>
              <label className="block text-xs font-medium text-[#6e6e73] mb-1.5">Archivo <span className="text-red-500">*</span></label>
              <div onClick={() => fileRef.current?.click()} className="w-full rounded-xl border-2 border-dashed border-black/12 dark:border-white/12 px-4 py-4 text-center cursor-pointer hover:border-blue-400 transition-colors">
                {file ? <p className="text-sm text-[#1d1d1f] dark:text-white">{file.name} <span className="text-[#6e6e73]">({formatSize(file.size)})</span></p>
                  : <p className="text-sm text-[#aeaeb2]">Haz clic para seleccionar .apk / .aab / .ipa</p>}
              </div>
              <input ref={fileRef} type="file" accept=".apk,.aab,.ipa" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
            </div>
          )}
          <div>
            ...
            <textarea value={releaseNotes} onChange={(e) => setReleaseNotes(e.target.value)} rows={2} placeholder="Correcciones y mejoras…"
              className="w-full rounded-xl border border-black/12 dark:border-white/12 bg-transparent text-sm px-3 py-2 text-[#1d1d1f] dark:text-white placeholder-[#aeaeb2] focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
          </div>
          {error && <p className="text-sm text-red-500">{error}</p>}
          <div className="flex gap-2 pt-1">
...
function VersionsPanel({ app, onClose, onUpload }: VersionsPanelProps) {
  ...
  useEffect(() => {
    setLoading(true);
    mobileAppService.listVersions(app.slug).then(setVersions).catch(() => {}).finally(() => setLoading(false));
  }, [app.slug]);
  ...
  async function handleActivate(id: string) {
    ...
    try {
      ...
      setVersions(updated);
    } catch { setError("No se pudo activar."); }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white dark:bg-[#111116] rounded-2xl shadow-2xl overflow-hidden max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-black/8 dark:border-white/8">
          <h3 className="text-base font-semibold text-[#1d1d1f] dark:text-white">
            {app.icon_emoji && <span className="mr-2">{app.icon_emoji}</span>}{app.name} — versiones
          </h3>
          <div className="flex items-center gap-2">
            <button onClick={onUpload} className="flex items-center gap-1.5 bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] text-xs font-medium px-3 py-1.5 rounded-lg hover:opacity-80 transition-opacity">
              <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" className="w-3 h-3"><path d="M8 2v12M2 8h12" /></svg>
              Nueva versión
            </button>
            <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-full hover:bg-black/8 dark:hover:bg-white/8 text-[#6e6e73]">
              <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-4 h-4"><path d="M3 3l10 10M13 3L3 13" strokeLinecap="round" /></svg>
        ...
        <div className="overflow-y-auto flex-1">
          ...
          {loading ? <div className="py-8"><Spinner /></div>
            : versions.length === 0 ? <EmptyState text="Sin versiones" sub="Sube la primera versión." />
            : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-black/8 dark:border-white/8">
                    {["Versión", "Plataforma", "Tipo", "Tamaño", "Estado", "Fecha", ""].map((h) => (
                ...
                <tbody>
                  {versions.map((v) => (
                    <tr key={v.id} className="border-b border-black/5 dark:border-white/5 last:border-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02]">
                      <td className="px-4 py-3 font-mono text-xs text-[#1d1d1f] dark:text-white">v{v.version}</td>
                      <td className="px-4 py-3 text-[#1d1d1f] dark:text-white">{PLATFORM_LABELS[v.platform]}</td>
                      <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/5 text-xs font-medium text-[#6e6e73] uppercase">{BUILD_LABELS[v.build_type]}</span></td>
                      <td className="px-4 py-3 text-[#6e6e73] text-xs">{formatSize(v.file_size)}</td>
                      <td className="px-4 py-3">
                        {v.is_active
                          ? <span className="flex items-center gap-1.5 text-xs font-medium text-[#34c759]"><span className="w-1.5 h-1.5 rounded-full bg-[#34c759]" />Activa</span>
                          : <span className="text-xs text-[#aeaeb2]">Inactiva</span>}
                      </td>
                      <td className="px-4 py-3 text-[#6e6e73] text-xs">{new Date(v.created_at).toLocaleDateString("es-ES", { day: "2-digit", month: "short", year: "numeric" })}</td>
                      <td className="px-4 py-3">
...
export function AdminAppsSection() {
  ...
  useEffect(() => {
    mobileAppService.listApps().then(setApps).catch(() => setError("Error al cargar apps.")).finally(() => setLoading(false));
  }, []);
  ...
  async function handleDeleteApp(id: string) {
    ...
    } catch { setError("No se pudo eliminar la app."); }
    finally { setDeleting(null); }
  }
  ...
  function handleAppSaved(app: App) {
    setApps((prev) => {
      ...
      return idx >= 0 ? prev.map((a) => (a.id === app.id ? app : a)) : [app, ...prev];
    });
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {error && (
        <div className="px-4 py-2.5 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900 text-sm text-red-600 dark:text-red-400">{error}</div>
      )}

      <Card>
        {loading ? <Spinner />
          : apps.length === 0 ? <EmptyState text="Sin apps" sub="Crea la primera con el botón de arriba." />
          : (
            <div className="grid gap-3 p-1">
              {apps.map((app) => (
                <div key={app.id} className="flex items-center gap-4 p-4 rounded-xl border border-black/8 dark:border-white/8 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                  <div className="w-10 h-10 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0" style={{ background: `${app.accent_color}20` }}>
                    {app.icon_emoji ?? "📱"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-semibold text-[#1d1d1f] dark:text-white">{app.name}</p>
                      <span className="text-[10px] font-mono text-[#aeaeb2] dark:text-[#636366]">{app.slug}</span>
                      {!app.is_published && (
                        <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400">Oculta</span>
                      )}
                    </div>
                    {app.description && <p className="text-xs text-[#6e6e73] dark:text-[#86868b] mt-0.5 truncate">{app.description}</p>}
                  </div>
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <button onClick={() => { setUploadForApp(app); setVersionsApp(null); }}
                    ...
                    <button onClick={() => handleDeleteApp(app.id)} disabled={deleting === app.id}
      ...
      {showNewApp && <AppModal onClose={() => setShowNewApp(false)} onSaved={handleAppSaved} />}
      {editingApp && <AppModal initial={editingApp} onClose={() => setEditingApp(null)} onSaved={handleAppSaved} />}
      {versionsApp && (
        <VersionsPanel
          app={versionsApp}
          onClose={() => setVersionsApp(null)}
          onUpload={() => { setUploadForApp(versionsApp); setVersionsApp(null); }}
        />
      )}
      {uploadForApp && (
        <UploadModal
