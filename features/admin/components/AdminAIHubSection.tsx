      ...
      <div className="relative space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className="text-sm text-[#6e6e73] dark:text-[#86868b]">
            {mcps.length} MCPs · {skills.length} Skills · {models.length}{" "}
            Modelos
          </p>
        </div>
          ...
          {[
            {
              ...
              icon: <IconMCP className="w-5 h-5 text-white" />,
            },
            {
              label: "Skills",
              count: skills.length,
              gradient: "from-pink-500 to-rose-500",
              icon: <IconSkills className="w-5 h-5 text-white" />,
            },
            {
              label: "Modelos",
              count: models.length,
              gradient: "from-cyan-500 to-blue-500",
              icon: <IconModel className="w-5 h-5 text-white" />,
            },
          ].map((s) => (
            <div
              key={s.label}
              className="relative p-4 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 overflow-hidden hover:shadow-xl hover:shadow-black/5 transition-shadow"
            >
              <div
              ...
              <div className="relative flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-black text-[#1d1d1f] dark:text-white">
                    {s.count}
                  </p>
                  <p className="text-[10px] text-[#aeaeb2] dark:text-[#636366] font-semibold uppercase tracking-wide">
                    {s.label}
                  </p>
                </div>
              </div>
          ...
          {[
            {
              id: "mcps" as const,
              label: "MCPs",
              icon: <IconMCP className="w-3.5 h-3.5" />,
            },
            {
              id: "skills" as const,
              label: "Skills",
              icon: <IconSkills className="w-3.5 h-3.5" />,
            },
            {
              id: "models" as const,
              label: "Modelos",
              icon: <IconModel className="w-3.5 h-3.5" />,
            },
          ].map((t) => (
            <button
              key={t.id}
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[1, 2, 3, 4].map((i) => (
              <div
                key={i}
                className="h-32 rounded-2xl bg-black/[0.03] dark:bg-white/[0.03] animate-pulse"
              />
            ))}
          </div>
        ) : (
            ...
            {tab === "mcps" && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {filteredMCPs.map((mcp) => (
                  <div
                    key={mcp.id}
                    className="relative p-4 rounded-2xl bg-white dark:bg-[#111116] border border-black/8 dark:border-white/8 hover:shadow-xl hover:shadow-black/5 transition-all group"
                  >
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${MCP_TYPES.find((t) => t.value === mcp.type)?.color ?? ""}`}
                        >
                          {mcp.type.toUpperCase()}
                        </span>
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${STATUS_COLORS[mcp.status] ?? ""}`}
                        >
                          {mcp.status}
                        </span>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          className="w-6 h-6 rounded-lg bg-black/5 dark:bg-white/8 flex items-center justify-center text-[#aeaeb2] hover:text-[#1d1d1f] dark:hover:text-white transition-colors"
                          type="button"
                        ...
                        <button
                          className="w-6 h-6 rounded-lg bg-red-500/10 flex items-center justify-center text-red-500 hover:bg-red-500/20 transition-colors"
                          type="button"
                          onClick={() =>
                            setDeletingId(deletingId === mcp.id ? null : mcp.id)
                          }
                        >
                          <IconTrash />
                        </button>
                        {deletingId === mcp.id && (
                          <div className="flex items-center gap-1">
                            <button
                              className="px-2 py-1 rounded-lg bg-red-500 text-white text-[10px] font-semibold"
                              type="button"
                            ...
                            <button
                              className="px-2 py-1 rounded-lg bg-black/5 dark:bg-white/8 text-[10px] text-[#6e6e73] dark:text-[#86868b]"
                              type="button"
                    ...
                    <p className="text-xs text-[#6e6e73] dark:text-[#86868b] line-clamp-2 mb-2">
                      {mcp.description}
                    </p>
                    {mcp.details && (
                    ...
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setDeletingId(
                              deletingId === skill.id ? null : skill.id,
                            )
                          }
                        >
                          <IconTrash />
                    ...
                    {skill.files.length > 0 && (
                      <div className="flex flex-col gap-0.5">
                        {skill.files.map((f, i) => (
                          <div
                            key={i}
                            className="flex items-center gap-1.5 text-[10px] font-mono text-[#aeaeb2] dark:text-[#636366]"
                          >
                            <span className="w-1.5 h-1.5 rounded-full bg-pink-500 shrink-0" />
                            <span className="truncate">{f.path}</span>
                            <span className="shrink-0 text-[9px] px-1 py-0.5 rounded bg-black/5 dark:bg-white/5">
                ...
                {filteredModels.map((model) => (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${PROVIDER_COLORS[model.provider] ?? "bg-gray-500/10 text-gray-600"}`}
                        >
                          {model.provider}
                        </span>
                        <span
                          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${STATUS_COLORS[model.status] ?? ""}`}
                        >
                          {model.status}
                        </span>
                        {model.is_default && (
                      ...
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          ...
                          onClick={() =>
                            setDeletingId(
                              deletingId === model.id ? null : model.id,
                            )
                          }
                        >
                          <IconTrash />
                    ...
                    {model.model_id && (
                      <p className="text-[10px] font-mono text-[#aeaeb2] dark:text-[#636366] mb-2">
                        {model.model_id}
                      </p>
                    )}
                    <div className="flex flex-wrap gap-1 mb-2">
                      {model.capabilities.map((cap) => (
                        <span
                          key={cap}
                          className="text-[9px] font-medium px-1.5 py-0.5 rounded bg-black/5 dark:bg-white/8 text-[#6e6e73] dark:text-[#86868b]"
                        >
                          {cap}
                        </span>
                      ))}
            ...
            <div className="p-6 space-y-4">
              ...
              {tab === "skills" && (
                <>
                  ...
                  <SelectField
                    label="Categoría"
                    options={SKILL_CATEGORIES.filter(
                      (c) => c.key !== "all",
              ...
              {tab === "models" && (
                <>
                  ...
                  <Field
                    ...
                    value={String(form.context_window ?? 0)}
                    onChange={(v) =>
                      setForm({ ...form, context_window: parseInt(v) || 0 })
                    }
                  />
                  <Field
                    label="Precio Input ($/1M)"
                    value={String(form.pricing_input ?? 0)}
                    onChange={(v) =>
                      setForm({ ...form, pricing_input: parseFloat(v) || 0 })
                    }
                  />
                  <Field
                    label="Precio Output ($/1M)"
                    value={String(form.pricing_output ?? 0)}
                    onChange={(v) =>
                      setForm({ ...form, pricing_output: parseFloat(v) || 0 })
                    }
                  />
              ...
              <button
                className={`px-5 py-2 rounded-xl text-sm font-medium text-white transition-all ${
                  saving
                    ...
                    : "bg-gradient-to-r from-violet-500 to-pink-500 hover:shadow-lg hover:shadow-violet-500/20"
                }`}
...
function Field({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold text-[#6e6e73] dark:text-[#86868b] mb-1">
        {label}
      </label>
      <input
