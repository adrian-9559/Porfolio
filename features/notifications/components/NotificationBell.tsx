...
export function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <svg className="w-4.5 h-4.5 text-[#6e6e73] dark:text-[#86868b]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {!loading && recent.length === 0 && (
              <div className="flex flex-col items-center py-10 text-center">
                ...
                <p className="text-sm text-[#6e6e73] dark:text-[#86868b]">Sin notificaciones</p>
              </div>
            )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <p className={`text-xs font-medium truncate ${!n.read ? "text-[#1d1d1f] dark:text-white" : "text-[#6e6e73] dark:text-[#86868b]"}`}>
                      {n.title}
                    </p>
                    <span className="text-[10px] text-[#aeaeb2] dark:text-[#636366] shrink-0">{timeAgo(n.created_at)}</span>
                  </div>
                  <p className="text-xs text-[#6e6e73] dark:text-[#86868b] mt-0.5 line-clamp-2">{n.message}</p>
                </div>
