...
export type ColumnType = "text" | "number" | "boolean" | "date" | "email" | "url";
...
export const dataTableService = {
  ...
  getRows: (
    ...
    return apiFetch<{ rows: DataTableRow[]; total: number; page: number; pageSize: number }>(
      `${BASE}/${id}/rows?${q}`,
    );
  },

  // Update cells
  updateCells: (id: string, rowId: string, cells: { columnId: string; value: string }[]) =>
  ...
  // Add row
  addRow: (id: string) => apiFetch<DataTableRow>(`${BASE}/${id}/rows`, { method: "POST" }),
  ...
  // Update column
  updateColumn: (id: string, columnId: string, data: { name?: string; type?: ColumnType }) =>
  ...
  // Delete table
  deleteTable: (id: string) => apiFetch<void>(`${BASE}/${id}`, { method: "DELETE" }),

  // Members
  listMembers: (id: string) => apiFetch<DataTableMember[]>(`${BASE}/${id}/members`),
  inviteMember: (id: string, data: { email: string; role: "editor" | "viewer" }) =>
